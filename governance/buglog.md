...
- **Root cause:** temp=0.0 ⇒ deterministic re-extraction — same prompt + same passage ⇒ same empty output. Re-running the same extractor will NOT fill them; needs a different prompt (explicit "if no steps/parameters/actors are enumerable, return NULL rather than an empty-typed shell") OR cross-examination fill (BUG-181#2 / D2471 recipe-builder path) OR relabel to principle/descriptive.
- **Fix (deferred):** (a) probe these 48 with Phi-4-mini (probe role) to confirm role mislabel vs genuine-thin; (b) add a "NULL-if-not-enumerable" instruction to the singleton/single-source extraction prompt; (c) otherwise accept-and-flag (sidecar-first, D2470) — NOT an S4 blocker.
- **Status:** 🟡 OPEN (deferred — does not block S4; tracked for post-S4 quality pass)
- **Source:** 2026-08-27 — D2479 targeted rerun verification
