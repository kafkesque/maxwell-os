...
>
> **✅ DONE 2026-08-28 (D2485) — pre-launch P0/P1/P2 hardening:**
> - **P0 S5 input fingerprint** — `stage5_verify.py` writes `checkpoint.jsonl.input_fingerprint.json` (S4 sha256 + schema/taxonomy/pipeline_commit + manifest_hash + model lineup) and HARD-DISCARDS any stale/mismatched checkpoint (closes the D2409 0-overlap blind spot; BUG-150 t11 was 93.3% fb_id overlap). Manual archive ritual eliminated.
> - **P1 BUG-186 `is_specialized`** — derived `= (depth == "specialized")` in `schemas.FB` + `stage4_merge.py`; dead LLM-classified read removed.
...
> - **MUST before S5 (remaining):** (1) **re-run S4** to recover the 5,322 FBs — NOTE: the `.depth.json` pre-pass is **deleted at completion**, so this is a FULL ~40-52h re-run, not ~35h; (2) retry the 7 failed clusters; (3) run `scripts/gate_emerging_rate.py` on the COMPLETE checkpoint (fail-closed).

> **🔎 ROUNDTABLE (2026-08-30, 3 forensic subagents) — latent silent-failure sites OFF the S4 critical path** (full detail in `governance/ROUNDTABLE_BUG188_FORENSIC_2026-08-30.md`). Active S2→S4→S5→S6 path is fail-closed. **✅ DONE (e4bd07e, 2026-08-30):** 7 non-atomic `open(...,"w")` writers to `safe_write_jsonl` (`bridge_s2_to_s4.py:68`, `stage1_5_fastembed.py:214`, `stage1_5_domain_cluster.py:236`, `probe_run.py:264`, `stage0_5_extract_metadata.py:400`, `golden_sampler.py:264`, `run_diagnostic.py:348`); fix 1 silent S0 resume reader (`stage0_convert.py:160-162`); fsync added (`benchmark_s4_depth_prompt_ab.py`, `benchmark_s4_depth.py`, `benchmark_s4_depth_frugal.py`); preflight checkpoint gate added (`scripts/preflight_checkpoint_check.py`) + 6-FB live smoke passed. **✅ DONE (D2497, 2026-08-30):** C16 config-read fallbacks (~15 sites now log to stderr: `stage4_merged_call.py`×10, `stage1_3_prefilter.py`, `stage2_extract.py`, `hybrid_gate.py`, `runner.py`, `stage6_commit.py`, `stamp.py`) + 4 HIGH silent data-loss (`stage1_5_intent.py:60`, `stage2_extract.py:2999/3028`, `stage6_okf_export.py:396`) — the last open forensic-audit #4a items. 149/149 tests green. **Roundtable audit now fully closed on code** (remaining open items are off-active-path one-off scripts + post-hoc fixers, non-blocking).
...
10. **BUG-150** discipline `emerging` re-measure on fresh S4 (taxonomy promotion gate). Baseline 38.4% (stale 2,830) / 15.5% (canary post-D2398). **✅ GATE SCRIPT SHIPPED (D2485):** `scripts/gate_emerging_rate.py` fails closed on `taxonomy.emerging_discipline_rate_max/emerging_domain_rate_max/emerging_unmapped_rate_max` (C12), and `taxonomy_match_method` now splits `emerging_real` (genuine gap) vs `emerging_unmapped` (empty/invalid raw label — no longer fabricates "emerging"). Run it after S4 completes. Do NOT promote against stale data (BUG-167 lesson).
...
12. **S4 graceful-pause handler (SIGINT/SIGTERM → `_write_s4_checkpoint` → clean exit)** — ✅ **DONE (2026-08-28).** `stage4_merge.py` now installs SIGINT/SIGTERM handlers that set `_INTERRUPT_REQUESTED`; the main loop + depth pre-pass check it at safe boundaries (between clusters / chunks) and call `_write_s4_checkpoint` + `_write_depth_checkpoint` before `sys.exit(130)`. Loss capped at 0 clusters (in-flight call completes first). The D2477 depth pre-pass is now INCREMENTALLY checkpointed to `.depth.json` (crash-safe, resume-aware) instead of the uncheckpointed ~1-2h block. **BUG-184 FIXED** (per-chunk retry + serial fallback + `depth_batch_max_tokens: 2048` + `thinking_budget=128`); **BUG-183 FIXED** (S2-input fingerprint in `.state.json`). Live-verified: SIGINT → checkpoint → resume skipped 2/3 clusters + reused pre-computed depth → completed run cleared sidecars. Full-run relaunch pending.

13. **P1.3 gpt-oss cross-family FLAG — A/B TEST REQUIRED before enabling** (D2469 wired default-off). A/B: run `stage2_relabel_extraction_type.py` on a frozen 50–100-record set WITH and WITHOUT `--cross-family-model`, compare label agreement; adopt cross-family only with ≥99% agreement AND measure the disagreement-flag rate (expect low — gpt-oss vs Qwen3 should agree on clear FORM cases). Gate: no auto-apply on disagreement (already wired). Do NOT enable in production until the A/B passes.
...
3. **BUG-165 — S4/S5/S6 stage-drift (CLOSED — D2480, moot):** the fix was always "re-run S4→S5→S6 downstream on the finalized S2"; nothing blocks it structurally. Re-run once on the current 13,604-record S2 (8,402 checkpoint + 5,202 singleton), filtered frontier 10,063. Do NOT run twice.
...
| G5 | **`is_specialized` persistence** — parsed-but-not-persisted (None × 278). | ✅ DONE (D2485) — derived `= (depth == "specialized")` in `schemas.FB` + `stage4_merge.py`; dead LLM-classified read removed (BUG-186). Prompt-strip deferred (merged-call JSON contract → A/B first). |
