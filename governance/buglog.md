...
## 🟠 BUG-183 — 2026-08-27 — S4 intra-stage resume re-loads a STALE completed checkpoint when cluster IDs overlap across a corpus change — OPEN (worked around, not fixed)
...
- **Workaround (applied):** moved the stale `stage4_merge/t11/` outputs (checkpoint + sidecars + visual.md) to `backup/stale_s4_t11_20260827_2350/` before relaunching — fresh run confirmed (`4861 + 5202` loaded, no "resuming" line).
- **Fix (deferred):** the resume block must ALSO compare S2 provenance (`pipeline_commit` / `pipeline_run_id`) against the S4 checkpoint's, and discard when they mismatch — not rely on cluster-ID overlap alone. Add a test: stale checkpoint + same cluster IDs → fresh start.
- **Status:** 🟠 OPEN (worked around; resume-provenance guard deferred — S4 full run is fresh and unaffected)
