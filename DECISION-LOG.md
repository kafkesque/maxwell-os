...
### D2340 — Model Registry Drift: gpt-oss/Phi stale as "verifier" vs DeBERTa-only code (2026-08-13)
**Category:** GOV
**Decision:** Three-way model-role drift. `stage5_verify.py` is DeBERTa-v3-large ONLY (D2298; Phi-4-mini "DELETED", `cross_family_verifier: null`). But three registries still list decoder verifiers: `config/pipeline_config.yaml` (`models.verifier: gpt-oss-20b`, `models.verifier_v2: Phi-4-mini`), `config/model_assignments.yaml` (`S5_VERIFIER: Phi-4-mini`), and `agent/session_seed.yaml` (`models.verifier: gpt-oss`, `models.verifier_v2: Phi-4-mini`). The truth: gpt-oss-20b is the **S4 classifier** (D2249, `VERIFY_MODEL` in `stage4_merge.py`), not the S5 verifier. Fix: rename `pipeline_config.yaml` `models.verifier` → `classifier` (gpt-oss) and delete/annotate `verifier_v2` (Phi) as removed per D2298; align `session_seed.yaml` and `model_assignments.yaml`; keep `models.nli` = DeBERTa as the sole S5 verifier. This extends B8/D2328 (which only fixed the runner's "Phi-4-mini" *description*).
**Files:** `config/pipeline_config.yaml`, `config/model_assignments.yaml`, `agent/session_seed.yaml`
**Status:** OPEN — P1 (config-only; no code path reads the stale verifier role, but an operator/agent cannot trust the registry).
...
**Status:** OPEN — P1 (run-scoping correctness; no data loss, but a corpus run launched with `--run-id` would silently write into `latest/`).
