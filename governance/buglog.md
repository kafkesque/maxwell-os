...
## 🟠 BUG-205 — 2026-09-02 — `source_diversity` inflated by near-duplicate book files (S1/S2 book-dedup gap) — OPEN
...
- **Fix (future S1/S2):** book-level identity dedup (author + normalized title, NFKC fold + download-source suffix strip), then recompute `source_diversity` from deduped books. Post-hoc re-derivable from `source_books`.
- **Files:** `pipeline/stage1_5_embed_cluster.py` / `stage2_extract.py` (book dedup), `pipeline/schema_accessor.py` (`source_diversity` derivation)
- **Source:** frontier-review validation 2026-09-02 (claude0041/chatgpt0041), D2506
