# Master Roundtable Evaluation Prompt — v6.1
## Golden Set DSPy Fine-Tuning Readiness Audit

**Authority:** D2234 — Post quality-blocker golden set v4.4 (73 examples, 75 FBs, EP:12, NH:12, DM:12)
...
| Evidence verbatim | All evidence passages now match `cluster_segments` exactly (190/190, 100%). Audit tool at `tools/audit_evidence_passages.py`. |
...
- Golden set: 73 examples (was 60)
- Author concentration: ALL authors ≤3 ✅
- Evidence: 100% verbatim (190/190 passages)
...
## THE GOLDEN SET: 70 Examples (v4.4)
...
| `descriptive_model` | 9 | 13% | 12-15 (target reached) |
...
c) Do source segment texts match evidence passages verbatim? (190/190 verified by `audit_evidence_passages.py` — re-verify any that look suspicious.)
...
The golden set now has: 39 causal (52%), 12 empirical (16%), 12 normative (16%), 12 descriptive (16%).
...
The golden set has 21 negatives covering 15 distinct failure modes (post D2226/D2232 cleanups).

a) Do all 21 negatives teach GENUINELY DISTINCT rejection patterns?
...
d) Is the negative-to-positive ratio (21:54 = 0.39:1) appropriate? Note: this drifted from the pre-D2232 ratio of 23:37 (0.62:1) because we added 20 positives and removed/repurposed 10 negatives. Is this a problem for DSPy?
...
e) **NEW (D2234):** Now that the golden set is 73 examples with 75 FBs, is the size adequate for a DSPy pilot? Or does the 21:54 negative:positive ratio create issues?
...
1. `config/golden/stage2_fewshot_convergent.yaml` — The 70-example golden set (v4.4, primary evaluation target)
...
3. **Check verbatim evidence.** `audit_evidence_passages.py` already verified 190/190 (100%). But re-check any that look suspicious — trust but verify.
...
5. **Evaluate the negative:positive ratio.** 21:54 = 0.39:1 is lower than the original 0.62:1. Is this sufficient for DSPy to learn rejection? Should we add more negatives?
