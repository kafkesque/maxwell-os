...
> **Updated:** 2026-09-03 | **Decisions:** D2000-D2521

## 🔴 TOP PRIORITY (OPEN — pending user decision / work)
1. **311-row deterministic repair (D2540)** — `UPDATE fbs SET discipline_raw=discipline, taxonomy_match_method='exact' WHERE discipline<>'emerging' AND (discipline_raw IS NULL OR TRIM(discipline_raw)='')`. Deterministic, seconds, behind C13 backup+integrity gate. READY to apply.
2. **Track B reclassification (D2540)** — 1,174 emerging-empty-raw FBs: `python3 pipeline/reclassify_merged_axis.py --where "discipline='emerging' AND (discipline_raw IS NULL OR TRIM(discipline_raw)='')" --apply --batch --harvest temp/trackb_full.jsonl` (~3.5h; batch 2× validated D2532). Await OMLX idle + user go.
3. **NLI entailment audit (D2540/D2541)** — run `pipeline/nli_label_audit.py` (Qwen3-Coder wrote it this session) across 7,995 to flag low-entailment labels; then sampled human review n≈367 (~3h) to MEASURE semantic error rate before any broad reclassification.
4. **OWL/SKOS axiom spec (D2541)** — formalize 105 canonical labels as disjointness (discipline ⊓ domain ⊑ ⊥) + allowed-label closure; wire deterministic reasoner as the cross-contamination guarantee.
5. **S1 contextual embeddings** — recall 1.000→0.967 / MRR→0.792 tradeoff. Decide adopt vs keep gated (`contextual_embed.enabled=false`).
6. **S3 HyDE A/B run** — harness implemented (`--hyde`); run `/opt/homebrew/bin/python3 pipeline/retrieval_benchmark.py --hyde` before flipping `hyde.enabled: true`.

## ✅ SOLVED THIS SESSION (moved to bottom — no longer open)
- **D2399 human review** — both candidates REJECTED (organizational change ✗ design systems; entrepreneurial management ✗ agentic architecture) + `d2399_promotions_frozen: true` (treadmill halted, 0 candidates).
- **S2 rerank enable** — ENABLED (`rerank.enabled: true` + `RERANK_ENABLED` flag + `search_hybrid` cross-encoder post-pass).
- **S3 HyDE harness** — implemented (`--hyde` + `hyde`/`hybrid+hyde` legs + config `hyde` block).
- **D2399 policy A** (freeze + both-axes guard + single-axis-only) — implemented + enforced.
- **BUG-198 part 2** — 6 recovered singletons re-injected (DB 7,867→7,873).
- **BUG-197** — deterministic kind-swap (3,694 FBs; 0 residual axis leaks).
- **Checkpoint drift** — S4 checkpoint re-synced to DB (7,873, 0 drift).
- **Forensic audit** — clean (0 null/0 empty/0 leak/0 invalid).

> **D2521 (this session):** D2399 BOTH-CANDIDATES REJECTED + PROMOTIONS FROZEN + S2 RERANK ENABLED + S3 HYDE HARNESS. (1) D2399 HUMAN REVIEW (user decision): BOTH post-BUG-197 candidates REJECTED as semantically invalid — organizational change (85) > design systems (19) [design systems = UI/web/visual design, 19 FBs verified; organizational change = scattered innovation/strategy/systems raw grab-bag, domains_raw-only]; entrepreneurial management (7) > agentic architecture (2) [business/startup mgmt vs multi-agent AI, zero relation]. Flipped taxonomy.d2399_promotions_frozen: true (kill-switch) → check_for_replacements() returns [] (0 candidates, verified). (2) S2 RERANK ENABLED (production): A/B MRR 0.708→0.739 (+4.4%); rerank.enabled: true + RERANK_ENABLED flag + search_hybrid cross-encoder post-pass (rerank param, graceful degradation). (3) S3 HYDE HARNESS IMPLEMENTED: retrieval_benchmark.py --hyde adds hyde + hybrid+hyde legs (_generate_hypothetical + lru_cache); config hyde block + HYDE_* flags. Config + code only (no DB mutation). decision total=505, active=446. 192/192 tests still green.
> **D2520 (this session):** CHECKPOINT-DRIFT FIX + FORENSIC AUDIT + E2E FINDING. BUG-197/BUG-198 mutated the DB directly → S4 checkpoint was stale (6,819 FBs pre-swap + 6 missing). Fixed via `scripts/sync_checkpoint_from_db.py` (7,873 records, 0 drift). Forensic audit clean (0 null/empty/leak/invalid, integrity ok, segments valid). Fresh exports: `parquet/fbs_export_20260902_final.jsonl` + `fbs_snapshot_20260902_final.parquet`. health 10/10, integrity 17/18, preflight PASS, 192/192 tests. e2e stage0 = SHRINK GUARD (safety feature, book-count mismatch, NOT a regression).
> **D2519 (this session):** D2399 POLICY A ADOPTED + BUG-198 PART 2 + BUG-197 DETERMINISTIC KIND-SWAP (all EXECUTED). (1) D2399 policy A: freeze until BUG-197 runs + never promote both-axes-emerging + only stable single-axis long-tails — implemented `taxonomy.d2399_promotions_frozen` + `d2399_allow_both_axes_emerging=false` + `_is_both_axes_emerging()` guard (filters change management 92/6 + human factors engineering 37/18); no promotions applied. (2) BUG-198 part 2: 6 singleton principles re-injected via targeted S4→S5→S6 (`scripts/reinject_bug198_singletons.py`, reuses real stage fns); 6/6 committed (1 PASS + 5 QUARANTINE), DB 7867→7873, integrity ok; fixed D2408-adjacent LaTeX-escape JSON gap. (3) BUG-197 deterministic kind-swap (`scripts/bug197_kind_swap.py`, LLM-free): 3694 FBs — 598 disciplines recovered + 1830 redundant leaks dropped + 374 domains recovered; 0 residual axis leaks; taxonomy_counts recounted. 192/192 tests, config-audit 0 drift.
> **D2518 (this session):** TURN-WIDE FORENSIC VERIFICATION + GOV-SYNC + S1/S2/S3 A/B + SIDE-CAR BACKFILL. (1) Verification all clean: integrity 10/10, config-audit 0 drift, SQLite ok, DB 7867 rows 0 drift (0 bad labels/0 null/0 missing stamps/provenance, unique fb_ids), checkpoint_enriched 7867 valid records intact. (2) FIXED broken test (D2517 displacement left stale "ml systems & infrastructure" in S4-GOLD-002) + cleaned 4 stale displaced refs (pipeline_config ×2, synonym_map, domain_disciplines). (3) S1/S2/S3 A/B on 30-query golden set: S2 rerank now IMPROVES MRR 0.708→0.739 (7-query gate was artifact); S1 contextual = tradeoff recall 1.000→0.967/MRR→0.792; S3 HyDE not implemented. vec_fbs_ctx backfilled. (4) 25 sidecar empty-shells backfilled (gemma): 16 full + 9 partial (outcome_metric/syntax not in source — BUG-182). NEXT: (1) BUG-198 part 2 live S4→S5→S6 re-inject (6 records ready) → (2) BUG-197 deterministic kind-swap (~4,774 domains_raw discipline-leak) → (3) D2399 policy: change management (92)>design systems (19) domain + human factors engineering (18)>agentic architecture (2) discipline → (4) S3 HyDE harness.
> **D2517 (this session):** D2399 CANDIDATE 1 APPROVED + APPLIED (urban planning 133 → canonical domain, displacing ml systems & infrastructure 18; cardinality 43 unchanged; taxonomy v5.4→v5.5). BUG-196 FIXED (normalize_fb_name slices title-cased list + ellipsis sentinel; casing-revert bug fixed; test locked). BUG-198 PART 1 (6 dropped singleton principles located, application backfilled cross-family via gemma, re-injection manifest written; part 2 targeted S4→S5→S6 deferred). GOLDEN SET 16→30 (14 clean PASS FBs verified live hybrid top-10; all 30 vector recall 1.000 — un-gates S1/S2/S3). NEXT: (1) BUG-198 part 2 re-inject (needs live generator+NLI) → (2) BUG-197 raw-leak deterministic kind-swap (~4,774 domains_raw discipline-leak; 1,519 recoverable discipline) → (3) D2399 next: change management (92)>design systems (19) + human factors engineering (18)>agentic architecture (2) → (4) re-run S1/S2 A/B on 30-query golden set → (5) 25 empty-shells.
> **D2516 (this session):** D2399 CANDIDATE 2 APPROVED + APPLIED. organizational theory (19) → canonical discipline, displacing weakest canonical social engineering (1) (D2378 closed-loop, discipline cardinality 61 unchanged). marketing research KEPT as domain alias (marketing & communications), NOT discipline. scripts/apply_d2399_organizational_theory.py (deterministic/LLM-free, DB backed up): 19 FBs discipline→organizational theory + 1 FB social engineering→emerging; social engineering marked displaced; idempotent recount. taxonomy v5.3→v5.4 (+meta.displaced_canonicals). Cascade surfaced NEXT: domain urban planning (118) [candidate 1 pending]; discipline human factors engineering (18) > agentic architecture (2). NEXT: (1) candidate 1 urban planning decision → (2) BUG-197 reclass batch (only LLM re-run) → (3) BUG-196/198 → (4) 25 empty-shells → (5) golden set ~30.
> **D2515 (this session):** LABEL NORMALIZATION + COMPOUND DECOMP + ALIAS + DEAD-CANONICAL CLEANUP. (1) NFKC+dash-fold normalize_label() (NFKC alone insufficient — added dash-fold U+2010..U+2015/U+2212→U+002D) wired into both matchers + counter. (2) split_compound() decomposes "X & Y" → constituent canonicals. (3) 29 marketing/advertising/branding/sales fragments ALIASED (17 marketing & communications, 8 brand identity, 4 editorial & advertising). (4) removed 5 count-0 disciplines (multimodal metaphor/composition & layout/geometry & proportion/iconography/narrative design); 66→61 disciplines; v5.2→v5.3. (5) 6018 FBs re-derived LLM-free (5976 compound, 713 NFKC, 0 discipline — all emerging-disciplines are BUG-197 domain-not-discipline). D2399 now clean: urban planning (118) + organizational theory (19). 187 tests green. D2514 (Drift F): taxonomy_counts 3×-inflation FIXED. NEXT: (1) candidate-2 decision (organizational theory vs marketing-research cross-axis) → (2) D2399 apply → (3) BUG-197 reclass batch → (4) BUG-196/198 → (5) 25 empty-shells → (6) golden set ~30.
> **D2513 (this session):** DRIFT C EXECUTED + D2399 surfaced. Demoted 9 disciplines→domains (marketing merged into marketing & communications; 8 new domains; kept typography/creative process/design thinking). Config 35→43 domains, 75→66 disciplines, v5.1→v5.2. 851 FBs re-derived (discipline→emerging + domain added, DB backed up). alias_map: 14 stale entries removed. D2399 EXECUTED → human_review_taxonomy.json 2 candidates; CRITICAL: candidate 1 (marketing & advertising 360) is fragmented-raw-label duplicate of marketing & communications (28 emerging aliases) → do NOT auto-promote; ALIAS via synonym_map.yaml. Candidate 2 (organizational theory 57 vs dead multimodal metaphor 0) defensible. 186 tests green. NEXT: (1) human review D2399 candidates + alias 28 fragmented marketing labels → (2) D2399 apply → (3) Drift D raw-alias dedup (189 unrelated) → (4) BUG-196/198.
> **D2512 (this session):** TAXONOMY DRIFT FORENSIC AUDIT — 4 drifts found, B+D fixed, C evaluated. BUG-207: "5,686 reclass" figure WRONG (emerging 2,080 = correct-honest; applied-practice disciplines 1,677; genuinely-non-canonical 114 = economics 108 + color theory 6). BUG-208 FIXED: YAML↔taxonomy_counts out of sync (economics/color theory/multimodal metaphor/social sciences not canonical in DB) → reconcile_canonical_status() wired into run_post_commit_taxonomy + applied live (4 transitions). BUG-209 EVALUATED (awaiting decision): marketing→DOMAIN, product design→DOMAIN, typography→DISCIPLINE; +9 more (design thinking/design strategy/project management/leadership/personal productivity/industrial design/information architecture/design systems/creative process). BUG-210 FIXED: 231 raw-alias cross-axis overlaps locked with CI (0 coercions verified). NEW Drift E: context_signals lists bare 'marketing' (not 'marketing & communications') + software engineering/philosophy as domain signals → never matches. 186 tests green (+4). NEXT: user decision on demote set → Drift C fix → BUG-197 (only if open-set re-judgment wanted) → D2399.
> **D2511 (this session):** RRF keyword-leg pollution BUG FIXED — search_hybrid fused the metadata leg into RRF even with NO domain/discipline/depth filter; search_keyword() then returned ORDER BY borp_score DESC ≈ ROWID (constant list) → "High Contrast Visual Design" at rank ~2 in EVERY query. FIX: keyword leg only when a filter is set (test_hybrid_rrf_keyword_leg_d2511.py). BENCHMARK HARNESS DELIVERED (#7) — config/golden/retrieval_queries.yaml (7 verified queries) + pipeline/retrieval_benchmark.py (recall@k/MRR/P@k fts/vector/hybrid). BASELINE: vector=hybrid recall 1.000 MRR 0.857 P@k 0.100; FTS 0.000 (vocab mismatch proves vector value). S2 reranker (bge-reranker-v2-m3, CACHED+VERIFIED, newer than large): A/B DROPS MRR 0.857→0.606 → GATED OFF. S1 contextual (contextualize_text + backfill --contextual + vec_table): A/B DROPS recall 1.000→0.857 → GATED OFF. 182 tests green (was 174). NEXT: enlarge golden set 7→~30 → re-run S1/S2 A/B → S3 HyDE. BUG-197 reclass → D2399 (blocked on reclass).
> **D2510 (this session):** BUG-205 a/an residual FIXED (drop a/an from _SPACE_SUBTITLE_SPLIT; committed DB ALREADY correct — bug was latent/D2507-introduced, no data re-resolve; 11/11 tests). BUG-197 prompt FIXED (discipline-vs-domain disjointness block in stage4_merged_call.py; re-classification = future-batch LLM run, NOT now). STACK AUDIT: Homebrew Python missing datasketch+pypandoc (PEP 668), system python missing enable_load_extension → split retained. RERANKER (llmfit): bge-reranker-large 560M + Qwen3-Reranker-4B candidates, not installed. NEXT: BUG-197 reclass → D2399 (after reclass) → BUG-196/198 → 25 empty-shells → S1/S2/S3 gated.
> **D2509 (this session):** M1 VECTOR SEARCH FIXED + 7867 embeddings backfilled (RESOLVES D2508 M1). Four bugs: (1) env (python.org SQLite lacks enable_load_extension → use /opt/homebrew/bin/python3); (2) DIMENSION CONTRACT (insert_embedding/search_vector packed raw bge-m3 1024d into float[512] — M1 latent); (3) read path (get_conn never loaded sqlite-vec → "no such module: vec0"); (4) query form (JOIN+MATCH broke KNN detection + k-before-status-filter starved PASS). FIXED: both embed paths → embed_texts_bge_m3 (512d L2-norm); get_conn loads sqlite-vec (graceful FTS fallback); search_vector → KNN subquery + oversample; new pipeline/backfill_embeddings.py. VERIFIED: 7867/7867 (0 orphaned); FTS=0 "bounce back after crisis" while RRF recovers "Collective Intelligence Recovery"; 174+5 tests pass. RRF KEPT (M2); S1/S2/S3 gated. NEXT: BUG-205 a/an opener → BUG-197 reclass → D2399 → 25 empty-shells.
...
| 9 | P3 | S4 classifier fix: enforce domain/discipline disjointness D2422 (BUG-197) | `stage4_merged_call.py` prompt + golden | ✅ prompt DONE (D2510 — disjointness block added); re-classification = future batch |
...
| 21 | P2 | stage2 golden `domain`/`discipline` metadata drift (~108 cross-kind/non-canonical labels, dormant) — migrate to canonical OR deprecate fields | `stage2_fewshot_*.yaml` | ✅ PARTIAL (D2506 — 45 alias→canonical canonicalized; 123 cross-kind/missing left as dormant) |
...
| 27 | P1 | S5 weak max-entailment rule (ChatGPT B2) — add contradiction veto + coverage + re-calibrate | `stage5_verify.py` `deberta_check` | ✅ DONE (D2507 — `_b2_majority_verdict` strict-majority + contradiction-veto from clean passages; config `b2_majority_ratio`) |
...
| 31 | P1 | **BUG-205**: S1/S2 book-level dedup — collapse near-duplicate book files (author+title NFKC + download-source strip) before `source_diversity` | `book_metadata.py` | ✅ DONE (D2507 + D2510 a/an fix; 962→910 source_ids; residual title-rename/co-author/truncation → post-S6 Stage 0.5) |
...
| 36 | P1 | **M1 vector search DEGRADED** — `vec_fbs` absent (python.org SQLite lacks `enable_load_extension`) → switch to Homebrew Python / `knowledge-pipeline` conda env + `pip install sqlite-vec` + backfill embeddings from `fbs.definition` | `stage6_commit.py` / env | ✅ DONE (D2509 — 4 bugs fixed: env + dim-contract + read-path + query-form; 7867/7867 backfilled via `/opt/homebrew/bin/python3 pipeline/backfill_embeddings.py`; RRF vector leg LIVE) |
