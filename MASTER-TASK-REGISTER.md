...
> **Updated:** 2026-09-02 | **Decisions:** D2000-D2506
...
| 27 | P1 | S5 weak max-entailment rule (ChatGPT B2) — add contradiction veto + coverage + re-calibrate | `stage5_verify.py` `deberta_check` | ⏳ PARTIAL (D2506 — evidence-garbage filter added; synthesis-entailment still OPEN, blocked on BUG-205) |
...
| 31 | P1 | **BUG-205**: S1/S2 book-level dedup — collapse near-duplicate book files (author+title NFKC + download-source strip) before `source_diversity` | `stage1_5_embed_cluster.py`/`stage2_extract.py` | ⏳ (blocks trustworthy B2 synthesis discriminator) |
| 32 | P1 | **B2 synthesis-entailment** — cross-passage entailment/majority (not single max-entail) + contradiction-veto only from clean passages | `stage5_verify.py` | ⏳ (after BUG-205; post-hoc S5 re-run) |
