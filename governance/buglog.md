# Maxwell OS — Buglog
> **2026-09-03 (D2540/D2541/D2542 — semantic-vs-structural audit + peer-review adoption + delegation boundary):** **BUG-220** — `delegate()` to Qwen3-Coder/Qwen3.8 fails with `error decoding response body` (DELEGATE-002 harness manifestation, NOT a server fault); DIRECT one-shot OMLX `/v1/chat/completions` curl WORKS (verified live: Qwen3.8 red-team returned a valid audit; Qwen3-Coder returned a draft with multiple correctness bugs). Verified ontology asymmetry: 105 canonical (61 discipline + 44 domain) vs 6,553 raw (1,391 + 5,162); 0 axis leaks, 0 unknown, 1,485 empty-raw (311 canonical + 1,174 emerging). REJECT full reclassification (~24–28h); ADOPT measure-first (311 deterministic + 1,174 LLM + NLI audit + n≈367 human sample + OWL/SKOS axioms).
> **2026-09-03 (D2533 — FORENSIC AUDIT + 5 pre-existing bugs catalogued + checkpoint-sync blindspot FIXED + Qwen3.8 verify):** Audit of re-classification prep fixed 6 self-defects (harvest crash-safety, backup fsync, C17 type hints, BATCH_SIZE_DEFAULT, `.lower()` mirror, config-audit registry) and catalogued 5 PRE-EXISTING issues → BUG-215 (1,097 empty `discipline_raw`, now REPAIRED by re-classification), BUG-216 (27 empty `domains_raw`), BUG-217 (mixed checkpoint stamps v5.1/v5.5), BUG-218 (9 sidecar empty-shells), BUG-219 (integrity checker FP on stdlib/scripts imports). BLINDSPOT FIXED: `--sync` (default ON) auto-runs `sync_checkpoint_from_db.py` + `sync_s5_checkpoint_from_db.py` after `--apply` (DB↔checkpoint drift, D2519/D2520 class). Qwen3.8: claim 1 script-safety PASS, claim 3 corpus-aware opt-in PASS, claim 2 golden axis-clean PASS-with-note (`organizational behavior` = taxonomy-design review, not contamination). integrity 17/18 (1 pre-existing FP), preflight/stress PASS, config-audit 0 drift, contract 5/5. Small apply validated (9 rows, gate clean).
> **2026-09-03 (D2532 + D2531 — BUG-197 re-classification prep + corpus-aware pass-rate opt-in):** Wrote `pipeline/reclassify_merged_axis.py` (discipline-only merged-axis re-classification of ~2,343 `discipline='emerging'` FBs; dry-run default; `--apply` requires C13 backup + integrity gate then atomic transaction; `--harvest` JSONL for BUG-150). A/B `stage4.batch_enabled` = batch 12.2s/FB vs seq 23.4s/FB (~2×), batch VALIDATED. CRITICAL findings — (1) re-classifying DOMAINS regresses valid canonicals → discipline-only (preserve domains); (2) corrected prompt WORKS but `CANONICAL_DISCIPLINES` is missing real labels (`data science`/`human factors engineering`/`design theory`/`financial technology`) so only ~25–37% resolve, rest are BUG-150 gaps. BUGFIX — `BATCH_CRIBS_CLASSIFY_SYSTEM` was missing D2422/BUG-197 disjointness + "DO NOT use emerging" rules (now aligned with merged prompt). Golden axis-cleanliness ENFORCED (7/7 clean + 2 new contract tests). `e2e.corpus_aware_pass_rate` added (opt-in, enabled=false): convergent actually 31.1% (19/61), single-source 27.4% (17/62) — corrects D2528 "convergent 0.80". FIXED pre-existing session_seed.yaml YAML parse error (unquoted `: true`). integrity 10/10, config-audit 0 drift, contract 5/5.
> **2026-09-03 (D2528 — e2e S5/S6 COMPLETE + 123-FB vector backfill + verify_pass_rate A/B + stage4_golden expansion):** S5 36 PASS/87 QUARANTINE (29.3% — single-source FBs genuinely unentailed, NOT a threshold bug). S6 123 committed (DB 7,995); vector embeddings re-backfilled (vec_fbs/vec_fbs_ctx both 7,995, 0 orphaned). BUG-214 FIXED (D2527, taxonomy hook tuple row_factory). validate_results 9/10 — verify_pass_rate FAILS at 80% (calibrated for convergent, not single-source). stage4_golden 5→7 (domain/discipline disjointness, BUG-197 at golden level).
> **2026-09-03 (D2526 — e2e S4 COMPLETE + elaboration VERIFIED + BUG-187/180/145 status reconciliation):** S4 e2e finished clean (123 principle + 28 PT + 12 TI = 163, 0 failed, 0 classification errors; depth {domain 96/specialized 20/cross-domain 7}; 1,709 edges, 0 isolated). Elaboration VERIFIED in committed S2 checkpoint: **123/123 principle NON-empty, 40/40 non-principle EMPTY (0 leak)** — D2524 fix holds end-to-end. Status reconciliation: BUG-187 FIXED (7 fields in schemas.FB 359-392, D2496), BUG-180 DONE (mlx_provider.py removed), BUG-145 FIXED (D2417 repair + prompt disambiguation). BUG-193 FIXED (D2525, CircuitOpenError import). S5 running (PID 31721).
> **2026-09-03 (D2523 — S2 speed root-cause + 6-FB vector backfill COMPLETE + DB-bloat prevention):** **S2 SPEED ROOT-CAUSE = MEMORY-PRESSURE DECODE COLLAPSE, NOT `burst_decode_mode`.** The 20-book S2 "4–11h" was an oMLX decode collapse (RAM 78.7% + 0.37 GB swap → 1.9 tok/s). Restart recovered RAM to 31.7% and decode to ~20–29 tok/s. `burst_decode_mode` A/B/C hot-applied (forged `omlx_admin_session` → `POST /admin/api/global-settings`): `balanced` ~20–29, `off` ~20–25, `aggressive` ~16–17 tok/s — **`aggressive` is SLOWER** (opposite of the code comment), `balanced` ≈ `off`; reverted to `balanced` (default). RULED OUT as throttle. S2 ETA re-measured ~100 FBs/hr → **~4–5 h remaining** (597 work units, ~160 done); bounded by 30B-MoE ceiling, not misconfiguration. **6-FB VECTOR BACKFILL COMPLETE** — `pipeline/backfill_embeddings.py` (full + `--contextual`) under Homebrew python → `vec_fbs` **7,867→7,873**, `vec_fbs_ctx` **7,867→7,873**, both = fbs (0 missing, 0 orphaned); the 6 BUG-198 FBs are now vector-searchable. **DB-BLOAT PREVENTION** (VACUUM deferred until e2e ends): bloat = S1 diagnostic created-then-DROPPED `ctx_v2`/`ctx_v3` vec tables in production `maxwell.db` (SQLite doesn't reclaim on DROP, 133→167 MB). Prevention: never run diagnostics against production DB (run-scoped `--db`); VACUUM after e2e (reclaims ~34 MB); e2e should use a SEPARATE run-scoped DB (retrieval doesn't filter by run_id); `backfill_embeddings.py --limit` NEVER on production (does `DELETE FROM vec` first).
> **2026-09-03 (D2522 — S1 contextual REJECTED + S3 HyDE REJECTED + full e2e relaunched + gov-sync):** **S1 CONTEXTUAL REJECTED** (keep `contextual_embed.enabled: false`) — 30-query A/B re-run: hybrid recall 1.000/MRR 0.708 vs hybrid-ctx 0.967/0.792; the +12% MRR is real but the recall loss is a genuine semantic failure (loses "how to build resilient distributed systems" → SRE Principles, drops to rank 14). Root cause at embedding level: the context prefix (`discipline|domains|name`, cap 200) for that FB is ~200 chars of broad multi-axis taxonomy that DILUTES the definition signal (cosine 0.588 bare → 0.533 contextual; name-only/name+discipline also fail to beat bare). **S3 HYDE REJECTED** (keep `hyde.enabled: false`) — `--hyde --rerank` A/B: hyde recall 0.967/MRR 0.642, hybrid+hyde 0.967/0.642 — REGRESSES BOTH recall AND MRR (below raw hybrid 0.708, far below rerank 0.739); FTS contributes nothing so hybrid+hyde ≡ hyde. **S2 RERANK CONFIRMED** (already enabled D2521) — recall 1.000/MRR 0.739, no recall loss; only production retrieval change that survives. **FULL E2E RELAUNCHED** — cleared stale e2e checkpoints (SHRINK GUARD, D2520) via safe_delete, fresh DB pre-e2e backup, `python3 pipeline/e2e_test.py` (20 books) running in background. **GOV-SYNC** — vec_fbs/vec_fbs_ctx stale at 7,867 rows vs fbs 7,873 (6 BUG-198 FBs lack vectors, re-backfill due); BUG-198 → RESOLVED (part 2 executed); BUG-197 → kind-swap executed + prompt fix verified (residual re-classification deferred); decisions.yaml re-synced (total 506, active 447).
> **2026-09-03 (D2521 — D2399 both-candidates REJECTED + promotions FROZEN + S2 rerank ENABLED + S3 HyDE harness + S5 checkpoint re-sync):** **D2399 HUMAN REVIEW RESOLVED (user decision).** BOTH post-BUG-197 candidates REJECTED as semantically invalid: `organizational change` (85 emerging domain) > `design systems` (19 canonical) — WRONG (design systems = UI/web/visual design [Figma/tokens/grids, 19 FBs verified]; organizational change = scattered innovation/strategy/systems raw grab-bag, domains_raw-only, NOT a design domain); `entrepreneurial management` (7 emerging discipline) > `agentic architecture` (2 canonical) — WRONG (business/startup mgmt vs multi-agent AI, zero relation). **ACTION: flipped `taxonomy.d2399_promotions_frozen: true` (kill-switch) → `check_for_replacements()` returns [] (0 candidates, verified).** Treadmill fully halted. **S2 RERANK ENABLED** (A/B MRR 0.708→0.739, +4.4%): `rerank.enabled: true` + `RERANK_ENABLED` flag + `search_hybrid()` cross-encoder post-pass (graceful degradation). **S3 HYDE HARNESS IMPLEMENTED** (`--hyde` + `hyde`/`hybrid+hyde` legs + config `hyde` block). **S5 CHECKPOINT RE-SYNCED** (user follow-up): the S5 checkpoint `stage5_verify/t11/checkpoint.jsonl` was MISSING (only `.stale_*` backups remained — BUG-197/198 mutated the DB directly after S5 ran, and `stage6_commit.load_stage5_fbs()` reads it). FIXED via `scripts/sync_s5_checkpoint_from_db.py` (deterministic, NO NLI re-run): re-synced 6,819 taxonomy fields + appended the 6 BUG-198 FBs (re-derived classify_model/is_specialized/manifest_hash/isor/epistemic_status/verification_method from the DB) → **7,873 records, 0 taxonomy drift**, expected_count updated, input fingerprint regenerated to match the re-synced S4 checkpoint. Config + code + S5 artifact only — no DB mutation.
> **2026-09-02 (D2520 — checkpoint drift FIXED + forensic audit clean + e2e shrink-guard note):** **CHECKPOINT DRIFT FOUND + FIXED** — BUG-197 kind-swap + BUG-198 re-inject mutated the SQLite DB directly, leaving `stage4_merge/t11/checkpoint_enriched.jsonl` STALE (7,867 vs DB 7,873; 6,819 FBs with pre-swap leaked taxonomy; 6 FBs missing). Fixed via `scripts/sync_checkpoint_from_db.py` → 7,873 records, 0 residual drift. **FORENSIC AUDIT clean** — 0 null required fields, 0 empty provenance, 0 axis leaks, 0 invalid canonical/depth/evidence/status, integrity ok, foreign-key clean, all source_segments valid 64-char hex. **E2E NOTE** — `just eval` stage0 fails with SHRINK GUARD when book count < existing e2e checkpoint (safety feature working correctly, NOT a regression). D2510 BUG-197 prompt fix verified present (stage4_merged_call.py:46-53).
> **2026-09-02 (D2519 — BUG-198 part 2 + BUG-197 deterministic kind-swap + D2399 policy A):** **BUG-198 RESOLVED** — 6 recovered singleton principles re-injected via targeted S4→S5→S6 mini-run (`scripts/reinject_bug198_singletons.py`, reuses real `merged_cribs_classify`/`classify_depth_focused`/`deberta_check`/`insert_fb`). 6/6 committed (1 PASS + 5 QUARANTINE — single-source singletons), DB 7867→7873, integrity ok, R14 stamps correct (v5.5/Qwen3-Coder/DeBERTa). Also fixed **D2408-adjacent LaTeX-escape JSON gap**: gpt-oss emitted `\(…\)` invalid JSON escapes (record "Monte Carlo PDE Solution Method") → `_clean_latex_json()` repair. **BUG-197 RESOLVED (deterministic, NOT LLM)** — `scripts/bug197_kind_swap.py` field-scoped UPDATE of domains/domains_raw/discipline/discipline_raw only: **3694 FBs** — 598 disciplines recovered (discipline='emerging' + single unambiguous discipline via `match_to_canonical` opposite-axis guard), 1830 redundant leaks dropped, 374 domains recovered (domain labels in discipline_raw). **VERIFIED 0 residual axis leaks** + integrity ok + taxonomy_counts recounted. Ambiguous both-axes-resolving labels left alone (never re-adjudicated). **D2399 policy A ADOPTED** — `taxonomy.d2399_promotions_frozen` (kill-switch) + `taxonomy.d2399_allow_both_axes_emerging=false` + `_is_both_axes_emerging()` guard filters axis-confused `change management` (92/6) + `human factors engineering` (37/18); no promotions applied. Post-BUG-197 pool: `organizational change` (85)>`design systems` (19) + `entrepreneurial management` (7)>`agentic architecture` (2) — human-review only. 192/192 tests pass.
> **2026-09-02 (D2518 — forensic verification + gov-sync + S1/S2/S3 A/B + side-car backfill):** **NEW FINDING + FIX — broken test from D2517 displacement.** `test_stage4_golden_contract.py::test_s4_golden_conforms_to_ontology` FAILED: `S4-GOLD-002` still referenced the now-displaced canonical domain `ml systems & infrastructure` (D2517 displaced it but did NOT propagate to the S4 golden fixture). FIXED: `S4-GOLD-002` domains → `systems & frameworks`. Also cleaned 4 other stale displaced refs: `pipeline_config.yaml` context_signals ×2, `synonym_map.yaml` `ml_systems_infrastructure` block (was dead — `_accept()` filtered it), `domain_disciplines.yaml` ml-systems block + 13 `social engineering` lines. **VERIFICATION (read-only, all clean):** integrity 10/10, config-audit 0 drift, SQLite `integrity_check` ok, DB 7,867 rows 0 bad labels / 0 null required / 0 missing R14 stamps / 0 missing provenance, fb_id unique, checkpoint_enriched.jsonl 7,867 valid records. **S1/S2/S3 A/B (30-query golden set):** S2 rerank now IMPROVES MRR 0.708→0.739 (was gated off on 7-query 0.606 regression = small-sample artifact); S1 contextual = tradeoff (recall 1.000→0.967, MRR→0.792); S3 HyDE not implemented. **25 side-car empty-shells backfilled** via gemma cross-family (`scripts/backfill_sidecar_empty_shells.py`): 16 fully, 9 partial (quantitative `outcome_metric` / tool `syntax`/`parameters` genuinely absent from source — BUG-182 model-level gap; those stay `body_incomplete: true` + reason).
> **2026-09-02 (D2515 — label normalization + compound decomposition + alias + dead-canonical cleanup):** **Drift G (BUG-212) + fragmentation + alias all EXECUTED.** (1) **NFKC + dash-fold** — `normalize_label()` in schemas.py (NFKC → fold U+2010..U+2015/U+2212 → U+002D → lowercase); wired into `match_to_canonical`, `_build_synonym_index`, `map_to_canonical_with_fallback` (stage4_merge), and `update_counts_from_fbs`/`_is_long_tail` (taxonomy_manager). Collapses "human‑computer" (U+2011) ≡ "human–computer" (U+2013) ≡ "human-computer" (U+002D); NFKC ALONE does NOT collapse dashes — dash-fold was the necessary addition. (2) **Compound decomposition** — `split_compound()` + decomposition in `match_domains_to_canonical` + stage4_merge domain loop + `_is_long_tail` (a compound whose parts all resolve is NOT a long-tail challenger). "marketing & advertising" → `marketing & communications` + `editorial & advertising`. (3) **Alias** — 29 marketing/advertising/branding/sales fragments added as raw aliases to taxonomy_v5.yaml (17→marketing & communications, 8→brand identity, 4→editorial & advertising). (4) **Dead-canonical cleanup** — removed 5 count-0 disciplines (multimodal metaphor, composition & layout, geometry & proportion, iconography, narrative design); domain_disciplines.yaml −78 refs; disciplines 66→61; version v5.2→v5.3. (5) **Data re-derived** — `scripts/normalize_taxonomy_labels.py` (LLM-free, backed up) → **6018 FBs**: 5976 compound domains decomposed, 713 NFKC edits, 0 discipline re-resolutions (all `discipline='emerging'` are BUG-197 domain-not-discipline, correctly untouched). D2399 now surfaces **clean** candidates: `urban planning` (118) > `ml systems & infrastructure` (18) + `organizational theory` (19) > `social engineering` (1). 187 tests green (5 new).
> **2026-09-02 (D2514 — Drift F: taxonomy_counts cumulative inflation FIXED + D2399 counts re-derived):** **BUG-211 (Drift F) FOUND + FIXED** — `update_counts_from_fbs()` counted `committed_at >= now-24h` and FELL BACK to a full scan that INCREMENTED `count = count + 1` on top of existing rows, so every fallback run re-added the whole corpus. Live proof: `marketing & advertising` read **360** in `taxonomy_counts` while the DB actually held **120** (exactly 3×); `marketing & branding` 339 vs 113; `advertising & marketing` 219 vs 73; `advertising & branding` 99 vs 33; `brand identity` 492 vs 164; `organizational theory` 57 vs 19. **Consequence:** D2399 candidates were computed on inflated numbers — the TRUE candidate 2 is `marketing research` (28) displacing dead `multimodal metaphor` (0), NOT `organizational theory` (19). **FIXED:** reset all `taxonomy_counts.count → 0` then idempotent full recount (7867 rows = trivial scan). Regression test `test_update_counts_is_idempotent()` added (5/5 taxonomy guards green). **D2399 candidate 1 NOT yet applied** — post-recount `marketing & advertising` (120) is a COMPOUND co-occurrence label overlapping canonical `marketing & communications` + `editorial & advertising`; recommending ALIAS-not-promote (see Drift F body).
> **2026-09-02 (D2513 — Drift C EXECUTED + D2399 fragmentation caveat):** **Drift C executed** — 9 applied-practice labels demoted discipline→domain (`marketing`→merged into `marketing & communications`; `product design`/`design strategy`/`project management`/`leadership`/`personal productivity`/`industrial design`/`information architecture`/`design systems`→8 new domains; kept `typography`/`creative process`/`design thinking`). Config 35→43 domains, 75→66 disciplines, 0 overlap, v5.1→v5.2. 851 FBs re-derived (discipline→emerging + domain added, DB backed up). alias_map: 14 stale entries removed. **D2399 EXECUTED but NOT auto-applied** — `human_review_taxonomy.json` surfaced 2 candidates; candidate 1 (`marketing & advertising` 360) is one of **28 fragmented emerging labels** all aliasing the `marketing & communications` domain just created → auto-promoting would recreate the redundancy Drift C removed. Human review (C8-G1) required; fragmented raw labels should be ALIASED via synonym_map.yaml, not promoted. 186 tests green.
> **2026-09-02 (D2512 — 4 taxonomy/config drifts found + B/D fixed + Drift C ontological audit):** Four verified, reproducible drifts (NOT hypothetical — each confirmed against live DB + config):
> **BUG-207 (Drift A — "5,686 re-classification" figure WRONG):** The number conflated 3 categories. Live DB: `discipline='emerging'` = 2,080 (CORRECT "honest unmapped" — model emitted a domain label, no discipline alias matched → `emerging`; NOT contamination); applied-practice disciplines (`marketing` 439 + `typography` 99 + `product design` 31 + `design thinking` 124 + `industrial design` 23 + `leadership` 61 + `project management` 104 + `design strategy` 127 + `personal productivity` 29 + `information architecture` 25) = 1,677 (taxonomy-DESIGN question, not data bug); genuinely non-canonical = `economics` 108 + `color theory` 6 (Drift B). A 2-day S4 re-run would re-process 2,080 already-correct records.
> **BUG-208 (Drift B — YAML ↔ `taxonomy_counts` OUT OF SYNC, breaks D2399):** `taxonomy_v5.yaml` has 75 discipline canonicals + 35 domain canonicals, but `taxonomy_counts` (what `check_for_replacements()` reads) has only 72 + 34. Missing from DB-canonical: `economics` (status='emerging', 108 FBs!), `color theory` (status='raw', 6), `multimodal metaphor` (discipline) + `social sciences` (domain). Root cause: `seed_from_taxonomy_yaml()` uses INSERT-OR-IGNORE and only runs when `existing_canonical==0`; labels promoted in YAML later (the "+4 promotions 2026-08-20") never get their status flipped; `update_counts_from_fbs()` auto-promotes raw→emerging but has NO emerging→canonical path. Consequence: D2399 would see `economics` (108) as `emerging` and PROPOSE PROMOTING a label already canonical in YAML (and demote a real discipline). **FIXED (D2512):** new `reconcile_canonical_status()` upserts canonical status from YAML every run + demotes DB-canonical labels absent from YAML.
> **BUG-209 (Drift C — prompt ↔ taxonomy CONTRADICTION):** `stage4_merged_call.py` (D2510) says "never emit `marketing`/`product design` as discipline", but `taxonomy_v5.yaml` line 987 lists `- canonical: marketing` and line 1577 `- canonical: product design` as DISCIPLINE canonicals (both promoted 2026-08-20). A re-run would have the model refuse a label the matcher still resolves to canonical → no-op or silent contradiction. Ontological verdict below (see Drift C).
> **BUG-210 (Drift D — 231 raw-alias cross-axis overlaps in config):** 231 raw aliases appear in BOTH a discipline entry and a domain entry (e.g. `advertising`→discipline `marketing` AND domain `editorial & advertising`). Verified: **0 cross-kind coercions** — the kind-scoped matcher (D2500 `_build_synonym_index(kind)` + `forbidden_keys`) already resolves each correctly per-axis. So this is NOT an active coercion bug; it is LATENT FUTURE-TAX (any future code reading raw aliases without the kind filter re-introduces the leak). **FIXED (D2512):** CI test `test_taxonomy_yaml_counts_sync.py` locks the invariant "0 cross-kind coercion from overlapping raw aliases" + "YAML canonicals ⊇ DB canonicals".
> **Drift C ontological verdict (D2512, evidence = live FB contents):** `marketing` → **DOMAIN** (FBs are branding/retail/advertising/campaign practice: "Logo Design As Identity Expression", "Guinness Pouring Ritual"); `product design` → **DOMAIN** ("User-focused Product Design", "Viral Product Design"); `typography` → **DISCIPLINE** (FBs are type design THEORY/evolution: "Type Design Evolution Through Technological Shift", "Formal Calligraphy Type Adaptation" — an academic field with scholarship, not a mere applied practice). Same audit flags 9 MORE applied-practice-as-discipline labels: `design thinking` (124), `design strategy` (127), `project management` (104), `leadership` (61), `personal productivity` (29), `industrial design` (23), `information architecture` (25), `design systems` (12), `creative process` (84) — see BUG-207 table. NOT yet acted (awaiting user decision on the full demote set).
> **D2512 FINAL DECISION (user):** all 12 disputed labels are DISCIPLINE canonicals (none domain, all canonical — verified live). FINAL demote set (discipline→domain): `marketing`, `product design`, `design strategy`, `project management`, `personal productivity`, `industrial design`, `information architecture`, `design systems` (8 agreed). KEEP as discipline: `typography`, `creative process`, `design thinking` (user's call). `leadership` = my recommendation DOMAIN (28/28 PASS FBs are applied management practice — "Mission Impossible Framework", "Leadership Accountability Return Process"; consistent with `project management` also demoted) — but flagged as the one user was uncertain about. **Drift E FIXED (D2512):** `context_signals` (stage4 + test.full_run) — folded bare `marketing` → `marketing & communications` (canonical domain); removed `software engineering` + `philosophy` (DISCIPLINE canonicals wrongly listed as domain signals). 186 tests green.
> **2026-09-02 (D2511 — RRF keyword-leg pollution FIXED + S1/S2 benchmark harness):** **RRF keyword-leg pollution (live bug, FIXED)** — `search_hybrid()` fused the metadata/keyword leg into RRF even with NO domain/discipline/depth filter; `search_keyword()` then returned `ORDER BY borp_score DESC`, which with `borp_score ≈ all 0.0` collapses to ROWID order = a CONSTANT list. "High Contrast Visual Design" (rowid 1) appeared at rank ~2 in EVERY query, diluting the real FTS+vector signal and making hybrid WORSE than vector. **FIX:** keyword leg participates ONLY when a filter is set (4-check regression test `test_hybrid_rrf_keyword_leg_d2511.py`). **S2 reranker A/B (bge-reranker-v2-m3, CACHED+VERIFIED)** — drops MRR 0.857→0.606 → **GATED OFF** (cross-encoder over-fits query-term overlap on 7-query golden set). **S1 contextual A/B** — drops recall 1.000→0.857 → **GATED OFF** (context prefix dilutes definition signal). Benchmark harness delivered (`config/golden/retrieval_queries.yaml` + `pipeline/retrieval_benchmark.py`). 182 tests green.
> **2026-09-02 (D2510 — BUG-205 a/an FIXED + BUG-197 prompt FIXED + stack audit):** **BUG-205 a/an residual FIXED** — dropped `a`/`an` from `_SPACE_SUBTITLE_SPLIT` (mid-title "a" in "Build a Large Language Model" vs "Build a Reasoning Model" was treated as subtitle opener). **VERIFIED committed DB is ALREADY correct** (distinct source_ids `92d597f3efa1cd12` vs `122fb1e8b4b66f17` — the buggy regex was introduced D2507 AFTER S4 merge, so it's LATENT, affects future re-extraction only; NO data re-resolve needed). 11/11 book-dedup tests (4 new). **BUG-197 prompt FIXED** — added explicit discipline-vs-domain disjointness block to `stage4_merged_call.py` PART 2 (graphic design/organizational behavior/data visualization/entrepreneurship/marketing/HR are DOMAINS not disciplines); re-classification (~2,080 emerging + 3,184 raw-leak) = future-batch LLM run, NOT now. **STACK AUDIT** — Homebrew Python has all pipeline deps EXCEPT datasketch+pypandoc (PEP 668); system python has all deps but no enable_load_extension → SPLIT retained (pipeline on system python, vector backfill+retrieval on Homebrew). **RERANKER (llmfit)**: bge-reranker-large (560M) + Qwen3-Reranker-4B (4.1B) candidates; not installed; S2 gated.
> **2026-09-02 (D2509 — M1 VECTOR SEARCH FIXED + 7867 embeddings backfilled):** D2508 "M1 degradation" was **FOUR bugs, not one**. (1) ENV — `/usr/local/bin/python3` SQLite lacks `enable_load_extension` → sqlite-vec never loads (fix: `/opt/homebrew/bin/python3`). (2) **DIMENSION CONTRACT (M1 latent) — `insert_embedding`+`search_vector` packed raw bge-m3 1024d into `vec_fbs float[512]`** (would silently misrank even after env fix). (3) READ PATH — `retrieve.get_conn()` never loaded sqlite-vec → `no such module: vec0`. (4) QUERY FORM — JOIN + `MATCH` + `k` broke sqlite-vec KNN detection (0 rows) AND applied k BEFORE the status filter (QUARANTINE 58% starved PASS to 0). **FIXED:** both embed paths → `embed_texts_bge_m3` (512d L2-normalized); `get_conn()` loads sqlite-vec (graceful FTS fallback); `search_vector` → KNN subquery + oversample (k=limit*5, floor 100); new `pipeline/backfill_embeddings.py`. **VERIFIED:** 7867/7867 backfilled (0 orphaned, 2m52s); FTS=0 for "bounce back after a crisis" while hybrid RRF recovers "Collective Intelligence Recovery" (semantic recall proven); 174 + 5 new M1 tests pass. **RRF KEPT (M2).** S1/S2/S3 remain gated.
> **2026-09-02 (D2508 — S6 COMMITTED 7867 + M1 vector degradation CONFIRMED + BUG-205 `a`/`an` false-merge):** S6 committed 7867 FBs (0 failed) + FTS + Parquet. **B2 majority MATERIALIZED**: PASS 5074→3274, QUARANTINE 2793→4593 (ENTAIL-majority 3274 / NEUTRAL 3727 / CONTRA-veto 293 / all-garbage 550 / MECH-FAIL 15 / evidence-garbage 8) — commit-with-status, NOT data loss; 626 needs_human_review. **M1 vector DEGRADED (NEW)** — `vec_fbs` absent → 0 embeddings: active Python (`/usr/local/bin/python3`, python.org framework 3.12.1) SQLite compiled WITHOUT `enable_load_extension`; `sqlite-vec` installed (`vec0.dylib` present) but `sqlite_vec.load()` raises `AttributeError`. Fix = Homebrew Python (`/opt/homebrew/bin/python3`, SQLite 3.53.3, load_extension=True) or `knowledge-pipeline` conda env (3.11.15, SQLite 3.52.0) + `pip install sqlite-vec` + backfill from `fbs.definition`. **BUG-205 residual false-merge** — `Build a Large Language Model` vs `Build a Reasoning Model` (Raschka, 2 DISTINCT books) collapse via `_SPACE_SUBTITLE_SPLIT` mid-title "a" opener → drop `a`/`an` from opener list (post-S6).
> **2026-09-02 (D2507b — BUG-206 shrink-guard ↔ fingerprint-reverify interaction FIXED):** `stage5_verify.py` D2485 "hard-discard and re-verify" path set `verified=[]` in memory but left the stale 100MB checkpoint on disk → the first incremental write (50 records) tripped `io_guard.safe_write`'s shrink guard (99.4% shrink → ValueError "partial-state overwrite"), aborting the legitimate full S5 re-run at record 50. **Fix:** archive the stale checkpoint + fingerprint to `.stale_<ts>` (R-D410-safe RENAME, never delete) before re-verifying. Post-hoc S5 re-run for D2507 (B2 majority rule) now proceeds.
> **2026-09-02 (D2507 — BUG-205 RESOLVED + B2 synthesis-entailment majority rule + retrieval research):** **BUG-205 FIXED** — `pipeline/book_metadata.py` `_unicode_fold()` (NFKC→NFKD→strip combining marks) + `_SPACE_SUBTITLE_SPLIT` (space-separated subtitle opener collapses with colon edition) + fallback-key sanitize; measured 962 files → 910 distinct canonical source_ids (16/16 tests). Residual (post-S6): same-book title rename / space-separated co-authors / truncated metadata titles → Stage 0.5 re-extraction + fuzzy matcher. **B2 synthesis-entailment FIXED** — `stage5_verify.py` `_b2_majority_verdict()` strict-majority (>0.5, config `b2_majority_ratio`) of CLEAN passages must entail to PASS / contradict to veto; replaces weak single-passage max-entailment (ChatGPT B2 / MTR task 32); score stays max(entail) for nli_calibrate (8/8 tests). `evidence_garbage_*` fallbacks promoted into config (C12). **RETRIEVAL RESEARCH** — `governance/RETRIEVAL-RESEARCH-2026-09-02.md`: MUST verify bge-m3 512d↔vec_fbs contract + keep RRF; SHOULD contextual/late-chunk + gated cross-encoder rerank + gated HyDE; WORTH USearch/TurboVec/LightRAG/ColBERT/GraphRAG/RAPTOR. Full suite 174 passed. **NEXT:** post-hoc S5 re-run → `just pre-s6` → `just stage6`.
> **2026-09-02 (D2506 — golden canonicalization + B2 evidence filter + quarantine triage + frontier-review validation):** **GOLDEN canonicalization** — `scripts/canonicalize_golden_metadata.py` surgically canonicalized **45** dormant `stage2_fewshot_convergent.yaml` domain/discipline alias→canonical values (line-level, production `match_to_canonical`, kind-safe; 123 cross-kind/missing left untouched); `.golden_meta.json` re-stamped; 9/9 contract tests + `golden_validate` 84/84 PASS. **B2 evidence-garbage filter** — `stage5_verify.py` `_is_garbage_passage()` excludes caption/title/UI fragments (<6 words, underscore-runs, symbol-dense) from `deberta_check()` before NLI; measured 205/2793 quarantined (7.3%) have ≥1 garbage passage, 154 (5.5%) all-garbage. **Quarantine triage** — `scripts/quarantine_triage.py` (50-record stratified sample) + `QUARANTINE-BATCH-ADJUDICATION-PROMPT.md` (frontier 7-label adjudication). **FRONTIER REVIEW (claude0041/chatgpt0041) — both findings TRUE:** (a) triage CSV was missing `evidence_passages` (only `evidence=cited`) → FIXED; (b) `source_diversity` INFLATED by near-duplicate book files (same book, different download source/edition) → **BUG-205** S1/S2 book-dedup gap; `source_diversity>=3` not trustworthy as B2 synthesis discriminator until fixed. Adopted Cohen-κ≥0.6 per-record agreement. **NLI distribution bimodal** (PASS median 0.716 vs QUARANTINE 0.016) — threshold recalibration RULED OUT. **GOV backfill:** D2505 decision entry (committed `6e621fd` but never synced) added.
> **2026-09-02 (D2504 — BUG-203/BUG-204 FIXED + `.golden_meta.json` contract + pre-S5 gate validation):** **BUG-203 FIXED** — S5 `FLAG` status was DEAD CODE (never assigned; stats init + `Disagree → FLAG (human): 0` lied about a D2322-removed human-review path) → DELETED FLAG from stats/icon + honest `Quarantined needing human review` count (schema `VERIFICATION_STATUS` keeps FLAG for other stages/legacy). **BUG-204 FIXED** — `TAXONOMY_MAX_DISCIPLINES` default 72→75 (live taxonomy_v5.yaml = 75); removed dead `MAX_DOMAINS`/`MAX_DISCIPLINES` + `_max_count` loop var (closed-loop preserves cardinality, cap never read — C19). **`.golden_meta.json` regenerated** (pipeline_commit→HEAD `9aa5fa5`, +`total_examples=84`/`convergent_positives=61`/`hard_negatives=23`; hash unchanged) + CI `tests/test_golden_meta_contract.py` (4 asserts). **Decision summary recomputed** 483→488 (+D2504). **FORENSIC correction** — final dedup was **6 collision groups (13→6 records, removed 7)**, NOT BUG-195's "4 groups (9 records)": 2 groups (`Compounding Habit Gains`, `Pineconing: Label and Address Fears`) were missed by the pre-finish partial-checkpoint audit. Record lineage reconciles **7880→7874 (−6 BUG-198)→7867 (−7 dedup)**; count = **7,867 CONFIRMED** (a reported "7,687" is digit transposition). **Pre-S5 gates all green**: integrity 10/10, config-audit no-drift, emerging-rate 26.4%/27.4%/0.0% PASS, `audit_s4_final` PASS, evidence-cleanliness 187 contaminated (8 severe→QUARANTINE), stress test PASSED.
> **2026-09-02 (D2503 — `ai systems` rename + BUG-202 fail-closed FIX + evidence audit):** Executed recommendation #1 for `ai systems`/`ai & agents` (do NOT merge — distinct): renamed canonical `ai systems`→`ml systems & infrastructure` across taxonomy_v5 + synonym_map + pipeline_config ×2 + domain_disciplines + stage4_golden S4-GOLD-002 + prompts/n5_modular + fix_golden_set + llm_golden_review_form; removed the overlapping `AI Systems` raw alias from `ai & agents` (case-variant collision in synonym index — now `match_to_canonical("ai systems")`→`ml systems & infrastructure` unambiguously). Post-hoc remap `scripts/rename_ai_systems_domain.py` → **15/7,867 records** (order-preserving, crash-safe, backup `backup_pre_ai_systems_rename_*`). **BUG-202 FIXED**: `_evidence_cleanliness_gate` `return set()`→`raise RuntimeError` (fail-closed); smoke-tested (module importable, happy path returns set()). **`audit_evidence_cleanliness.py` on live checkpoint: 187 contaminated (2.4%), 8 severe (>15%) → QUARANTINE.** 23/23 golden+taxonomy+disjointness+isolation tests green.
> **2026-09-02 (roundtable S4/S5 bulletproof audit — Claude+ChatGPT verdicts verified against live repo):** Claude=CONDITIONAL-GO, ChatGPT=NO-GO. Claim-by-claim verification: **~10/12 claims TRUE, 1 FALSE/mischaracterized (ChatGPT P1-5 "Color Psychology cross-axis" — real but it's 231 cross-axis raw aliases, NOT a matcher bug; kind-scoped index already disambiguates; root cause = open-set B3/BUG-197), 1 partial (P1-4 "emerging repair" = D2066/D2138 DESIGN + D2485 emerging_real distinction, not silent).** NEW BUGS logged: **BUG-202 `_evidence_cleanliness_gate` (stage5_verify.py:559) catches Exception → `return set()` = FAIL-OPEN** despite "C16 fail-loud" comment — contaminated evidence could reach NLI (mitigated by standalone `audit_evidence_cleanliness.py` pre-S5 gate in `just` chain). **BUG-203 S5 `FLAG` status is DEAD CODE** (never assigned; stats init + print "Disagree→FLAG (human): 0" misleadingly implies a human-review path). **BUG-204 `TAXONOMY_MAX_DISCIPLINES` default 72 vs live 75** (pipeline_paths.py:295) — stale cap, imported by taxonomy_manager but `_max_count` loop var unused → no functional blast radius today. Also verified: `deberta_check` DOES call `_nli_pair_scores(raise_on_error=True)` → `NLIInferenceError` → QUARANTINE (so ChatGPT-B2/gemma's "silent (0,0,0)" claim is only the nli_calibrate path, NOT production). NEW quantified finding: **231 cross-axis raw aliases** (raw labels in BOTH domain+discipline raw lists) + **`machine learning` emitted as raw DOMAIN 48×** (kind-safe fix parks → domain=`emerging`, no coercion) = the BUG-197 re-classification surface. `ai systems` (15 canonical) vs `ai & agents` (66) are SEMANTICALLY DISTINCT (agentic vs ML-infra), NOT duplicates. Local gemma cross-check UNRELIABLE on line-level specifics (confabulated "golden injection fails-open" at :345 — it's actually `raise RuntimeError` fail-closed at 85/314/523).
> **2026-09-02 (golden/few-shot forensic audit, post-D2500 cross-kind fix):** Golden↔taxonomy↔config↔script cross-reference. **(1) CLEAN — `config/golden/stage4_golden.yaml` (the ONLY classification few-shot injected into S4) is 100% canonical + cross-kind-free; D2500 did NOT corrupt it.** **(2) BUG-201 FIXED — `stage4.golden_max_examples: 4` silently DROPPED `S4-GOLD-005` (D2482 cross-domain boundary example) while `meta.total_examples: 5` and the D2483 A/B benchmark measured all 5 (3/5→5/5). Root cause: config truncation `[:S4_GOLDEN_MAX_EXAMPLES]` not re-synced when the 5th example was authored 2026-08-28. Fix: bumped to 5 + CI guard `test_s4_golden_not_silently_truncated`.** **(3) DORMANT DRIFT (P2, logged not rewritten) — `stage2_fewshot_*` top-level `domain`/`discipline` metadata fields carry ~108 cross-kind/non-canonical labels (`domain: typography/software engineering/semiotics/psychology/economics/marketing/engineering`, `discipline: graphic design/organizational behavior`, etc.), but these fields are NOT consumed by any code (formatters omit them; no reader in dspy_trainer/recall/probe/mining) → zero runtime cascade, but a future-tax blind spot if any code starts reading golden `domain`.** **(4) GAP — `config/alias_map.yaml` (215 discipline aliases, Phase 0b) was UNTRACKED in git.** Observation: `ai systems` vs `ai & agents` both canonical domains, both taught by golden = semantic near-duplicate.
> **2026-09-01 (post-S4 forensic audit + Phase 0 BUG-150 recurrence guard):** Full forensic audit of live t11 checkpoint (6,715 FBs, 63 keys 100% present, stamps 100% consistent). **NEW CRITICAL — BUG-195 fb_id collision → S6 silent data loss:** 4 collision groups / 9 records / 5 rows silently overwritten by `INSERT OR REPLACE` (`fb_id TEXT PRIMARY KEY`). Root cause: D2069 name-collision " (N)" suffix preserves the S2 fb_id (D2350) + D2120 dedup is per-cluster so cross-cluster identical (name, definition) survive. gemma cross-family audit confirms all 9 are TRUE duplicates (same concept, different cluster). **NEW — BUG-196 FB name truncation:** `FB_NAME_MAX_WORDS=8` still truncates mid-word (`"Situational Analysis in Practice Mapping Research with Grounded"` missing "Theory"; `"…Focused Execution to"`). **Phase 0 built + stress-tested (idempotent + fail-closed):** `scripts/dedup_fb_id.py` (merge collisions → fb_id unique; 6,706/6,706 non-collision records byte-identical; exit 1 on genuine hash collision), `scripts/remap_emerging.py` (Phase 0a Unicode fold + 0b alias map; discipline emerging 36.2%→25.5%; fb_id drift 0 / *_raw drift 0 / taxonomy_version unchanged; exit 2 on non-canonical alias), `scripts/s4_post_finish.py` (crash-safe unattended orchestrator: wait→validate→backup→dedup→remap→gate→report, stops before S5/S6), `config/alias_map.yaml` (215 validated discipline aliases). **BUG-188/D2487 truncation fix CONFIRMED ALREADY APPLIED** (safe_write_jsonl + load_jsonl self-check at stage4_merge.py:1165-1166). domain-not-discipline confirmed by LLM: graphic design 134, organizational behavior 107, data visualization 52, entrepreneurship 30, image compression, human resource management.
> **2026-08-31 (BUG-150 post-S4 audit + D2399 challenger-pool FIX):** `update_counts_from_fbs` long-tail gap FIXED — it read only canonical `domains`/`discipline` and skipped `"emerging"`, so `*_raw` long-tail labels never accumulated → D2399 promote/demote had an empty challenger pool. Now reads `domains_raw`/`discipline_raw` and counts any raw label mapping to no canonical (kind-constrained `get_synonym_index`, D2133). Dry-run on forked checkpoint (1,850 rec): `organizational behavior`→31 + `graphic design`→41 auto-promoted to `emerging`; `psychology`→233 canonical preserved (no double-count). **New finding:** domain-not-discipline mis-filing broader than graphic design — `entrepreneurship` (canonical DOMAIN) also emitted as DISCIPLINE (9 rec), extends BUG-150/BUG-167/D2422 cross-kind class. Unicode dash variants (`human–computer interaction` U+2013 ×9, `human‑computer interaction` U+2011 ×8) still in live pre-remap checkpoint → Phase 0a NFKC collapse (logged `s5_remap_ordering.md`). Live S4 healthy (PID 1933, resume `--only-fb-ids`, ~[1820/9995], 0 error lines).
> **D2497 (2026-08-30):** C16 config-read silent-fallback sweep + BUG-189/190/191 + Q4-vs-Q8 A/B + S4 model research (NO S4 rerun, per user). C16: ~15 config-read fallback sites now LOG to stderr instead of silently returning a hardcoded default — `stage4_merged_call.py` (10 model/max_tokens/batch_size/budget sites), `stage1_3_prefilter.py` (`drop_patterns_extra`), `stage2_extract.py` (`code_markers`), `hybrid_gate.py` (`s2.gate`), `runner.py` (`stages.timeouts`), `stage6_commit.py` (`version.yaml`), `stamp.py` (`pipeline_manifest`). BUG-189 FIXED (archived 3 dead `tools.pipeline_paths` phantom-import tools: protect.py + nuke_anytype.py + validate_book_structure.py; `get_space_id` confirmed NOT needed by stage6b — uses `route_space()`). BUG-190 FIXED (OMLX `/health` vs `/v1/models` catalog lie). BUG-191 FIXED (`check_stage_order()` counting `timeouts` as a stage). **Q4-vs-Q8 A/B**: Q4 = 14.41s/62.5% vs Q8 = 5.09s/93.8% → Q4 2.8× slower + 31pt less accurate; **keep gpt-oss-20b-MXFP4-Q8** (Q2 does not exist for gpt-oss-20b in MLX). **S4 model research** (`governance/S4_MODEL_RESEARCH_2026-08-30.md`): every last-month frontier model is a 100B–1.6T MoE that won't fit 64 GB; no classification-specific fine-tune exists; no swap warranted. 149/149 tests green. decisions.yaml → 482.
> **D2496 (2026-08-30):** MUST+SHOULD+WORTH pre-rerun hardening (NO S4 rerun, per user). MUST: (1) authoritative `--expect-count` — S4 persists S2-derived `expected_fb_count` sidecar (`<checkpoint>.expected_count.json`, survives completion); S5 `_preflight_gate` passes `--expect-count`; `preflight_checkpoint_check.py` now checks it INDEPENDENTLY of the self-referential manifest → a silent S4-side record DROP (clusters skipped, not truncated) fails closed. (2) BUG-187 schema drift FIXED — 7 fields added to `schemas.FB` (`source_segments`, `evidence_passages`, `evidence_passages_shown`, `is_summary`, `classification_error`, `classify_model`, `manifest_hash`) + `jargon` ALWAYS emitted + `content_types.yaml` `metadata.stamps` += `classify_model`/`taxonomy_version`/`manifest_hash`. (3) BUG-181#1 evidence gate WIRED into S5 — severe EPUB→MD evidence contamination (>0.15 artifact ratio) QUARANTINES before NLI (no verify-against-garbage). SHOULD: 3 silent-exception sites logged (C16): `stage0_convert` Docling fallback, `stage1_5_embed_cluster` prefilter flag, `stage6_okf_export` verification_results parse; memory-budget labels pinned in AGENTS.md (`delegate_model_budget` ~24GB / `omlx_guard_ceiling` 55GB / `run_wall_budget` 48GB). WORTH: 10 non-atomic auxiliary writers → `safe_write`/`safe_write_jsonl` (calibrate, coverage_check, nli_calibrate, protect, route, run_diagnostic, run_monitor, taxonomy_manager, stress_test_s2_exhaustive, test_matryoshka_nli). 149/149 tests green. NEW DRIFT FINDING → BUG-189.
...
## 🔴 BUG-215 — 2026-09-03 — 1,097 FBs `discipline='emerging'` with EMPTY `discipline_raw` (D2378 raw-preserve violation) — BEING REPAIRED by re-classification (D2532/D2533)
- **Root cause:** gpt-oss emitted a domain label as discipline (BUG-197) or nothing; `map_to_canonical_with_fallback` returned "emerging" with no raw label preserved → `emerging_unmapped` (D2485: fabrication risk, must stay ~0).
- **Fix:** `reclassify_merged_axis.py` now writes the corrected non-empty raw label for EVERY classified FB (resolved + still-emerging), so `discipline_raw` becomes non-empty and `taxonomy_match_method` → `emerging_real` (genuine gap) instead of `emerging_unmapped`.

## 🔴 BUG-216 — 2026-09-03 — 27 FBs `emerging` in `domains` but empty `domains_raw` (D2378) — OPEN
- **Root cause:** domain-axis analog of BUG-215. Out of scope for the discipline-only re-classification.
- **Fix:** separate domain-axis repair (re-decide domains) or accept (27 of 7,995).

## 🔴 BUG-217 — 2026-09-03 — t11 checkpoint has MIXED stamps (`taxonomy_version` v5.1/v5.5; `manifest_hash`; `pipeline_commit`) — OPEN
- **Root cause:** checkpoint re-synced across multiple pipeline versions (D2519/D2520) without re-stamping every record.
- **Fix:** re-stamp pass or accept — provenance metadata only; the discipline/domains axis is unaffected.

## 🟡 BUG-218 — 2026-09-03 — 9 sidecar empty-shells remain (5 PI + 1 PT + 3 TI) — OPEN (residual of D2518/BUG-182)
- quantitative `outcome_metric` / tool `syntax`/`parameters` genuinely absent from source text (model-level gap).

## 🟡 BUG-219 — 2026-09-03 — integrity check #4 flags stdlib `unicodedata`/`heapq` + deferred `scripts/audit_evidence_cleanliness` as "undeclared" — OPEN (checker false-positive)
- **Root cause:** integrity checker's stdlib allowlist is incomplete, and it doesn't scan `scripts/` for deferred imports. All 3 are real, resolvable imports — not drift.
- **Fix:** extend the checker's stdlib set + `scripts/` path. Non-blocking.

## 🟢 BUG-212 — 2026-09-02 — Unicode dash variants split the same label into N rows (Drift G) — FIXED
- **Symptom:** `human‑computer interaction` (U+2011 non-breaking hyphen) and `human–computer interaction` (U+2013 en dash) accumulated as SEPARATE `taxonomy_counts` rows and never matched the canonical `human-computer interaction` (U+002D) — the same discipline split across 3 spellings. Same for `e‑commerce`/`e-commerce` and ~150 other dash variants (713 FBs affected).
- **Root cause:** label matching used `str.strip().lower()` only — no unicode normalization. **NFKC alone is insufficient** (verified: NFKC keeps U+2010/U+2011/U+2013 non-ASCII; only U+2011→U+2010). Required an explicit dash-fold (U+2010..U+2015, U+2212 → U+002D) in addition to NFKC.
- **Fix (D2515):** `schemas.normalize_label()` = NFKC → dash-fold → strip → lower; applied to the synonym index, both matchers (`match_to_canonical`, `map_to_canonical_with_fallback`) and the counter (`update_counts_from_fbs`, `_is_long_tail`). Data re-derived post-hoc (NFKC edits 713 FBs).
- **Status:** FIXED + test-locked (`test_label_normalization_d2515.py`).

## 🟢 BUG-214 — 2026-09-03 — taxonomy post-commit hook crashes (`tuple indices ... not str`) — FIXED (D2527)
- **Symptom:** S6 e2e logged `⚠️ Taxonomy post-commit hook failed (non-fatal): tuple indices must be integers or slices, not str`. The D2066 `run_post_commit_taxonomy` recount silently never ran.
- **Root cause:** `stage6_commit.py` creates the conn with the default tuple row_factory; `reconcile_canonical_status()` uses `row["label"]`/`existing["status"]` (needs sqlite3.Row).
- **Fix:** `conn.row_factory = sqlite3.Row` at the top of `run_post_commit_taxonomy`. VERIFIED re-run on a fresh tuple-factory conn → no error.
- **Files:** `pipeline/taxonomy_manager.py`

## 🟢 BUG-213 — 2026-09-02 — compound co-occurrence labels ("marketing & advertising") accumulated as fragmented D2399 challengers — FIXED
- **Symptom:** Stage 4 emitted `X & Y` compound labels for FBs spanning two domains; these never matched a single canonical, so they parked as `emerging` and D2399 proposed PROMOTING them as new canonicals (recreating redundancy).
- **Fix (D2515):** `split_compound()` decomposes `X & Y` / `X and Y` into constituents, each resolved to its own canonical (an FB may legitimately span two domains). Wired into `match_domains_to_canonical`, the stage4_merge domain loop, and `_is_long_tail` (covered compounds are not challengers). 29 single-word fragments aliased directly. 5976 FBs re-derived.
- **Status:** FIXED + test-locked. D2399 now surfaces clean single-concept challengers (`urban planning`, `organizational theory`) instead of the marketing fragmentation.

## 🟢 BUG-211 — 2026-09-02 — taxonomy_counts cumulative inflation (Drift F): D2399 promote/demote ran on 3×-inflated counts — FIXED
- **Symptom:** `taxonomy_counts` held `marketing & advertising = 360`, `marketing & branding = 339`, `advertising & marketing = 219`, `advertising & branding = 99`, `brand identity = 492`, `organizational theory = 57` — but the live `fbs` table (PASS + QUARANTINE) actually held 120 / 113 / 73 / 33 / 164 / 19 respectively.
- **Root cause:** `update_counts_from_fbs()` (D2399 challenger-pool fix) selected `committed_at >= now-24h`; when that returned 0 rows it FELL BACK to a full `SELECT … FROM fbs` scan and ran `UPDATE … SET count = count + 1` — an increment, not a recount. Each fallback run re-added the whole corpus (≈3 runs observed). The counts column had no reset path.
- **Impact:** `check_for_replacements()` (D2399) compared emerging labels against a canonical floor using these inflated numbers. Two live consequences: (1) candidate 1 `marketing & advertising` looked 3× stronger than reality; (2) candidate 2 was reported as `organizational theory` (57) when the true strongest discipline challenger is `marketing research` (28) — the drift REORDERED the challenger pool.
- **Fix (D2514):** `update_counts_from_fbs()` now `UPDATE taxonomy_counts SET count = 0` then does an idempotent full recount of ALL FBs (drops the 24h window; 7867 rows is a trivial scan). `reconcile_canonical_status()` still runs first so status stays YAML-true. Regression guard `test_update_counts_is_idempotent()` (two calls → identical snapshot).
- **Status:** FIXED + test-locked. NOT auto-applied D2399 candidate 1 — post-recount the "marketing & advertising" label is a COMPOUND co-occurrence (FBs tagged marketing+advertising simultaneously), overlapping existing canonicals; see D2399 fragmentation caveat.

## 🟢 BUG-200 — 2026-09-02 — static taxonomy cross-kind raw-alias pollution: `match_to_canonical` COERCES a discipline label into a domain canonical (D2422 enforced on canonicals only, not aliases) — FIXED (code filter + matcher short-circuit + post-hoc re-derive)
- **Symptom:** `match_to_canonical('typography','domain')` → `'graphic design'`; `match_to_canonical('software engineering','domain')` → `'engineering practice'`; `match_to_canonical('machine learning','domain')` → `'ai & agents'` — all three are canonical DISCIPLINES silently coerced into DOMAIN canonicals.
- **Root cause (verified 2026-09-02):** TWO cross-kind sources. (1) `config/taxonomy_v5.yaml` raw-alias lists: **36 domain `raw:` aliases ARE discipline canonicals** (typography, color theory, software engineering, machine learning, product design, sociology, law, human-computer interaction, …) + **2 discipline `raw:` aliases ARE domain canonicals** (creative technology, motion design). (2) `config/synonym_map.yaml`: **9 domain synonyms ARE discipline canonicals** (product design, leadership, artificial intelligence, machine learning, software engineering, systems thinking, semiotics, communication theory, research methodology). `_build_synonym_index(kind)`'s `_accept` filter validated the TARGET canonical but NOT the SOURCE alias, so the kind-constrained index AND the flat index both inherited cross-kind aliases → `match_to_canonical` coerced instead of returning None. `tests/test_taxonomy_disjointness.py` only checked canonical↔canonical (D2422), never alias↔canonical.
- **Impact:** 2,455 records had a discipline canonical emitted into `domains_raw`; of those **2,324 (94.7%)** ended up with a NON-empty canonical domain (silent coercion: `product design`→`digital product`, `software engineering`→`engineering practice`, `sociology`→`social sciences`, `artificial intelligence`→`ai & agents`). This was the ROOT of the whole raw/canonical contamination chain (BUG-197/199).
- **Fix applied (2026-09-02):** (1) `pipeline/schemas.py` — `_build_synonym_index` now skips raw/synonym/keyword aliases whose lowercase form is a canonical of the OPPOSITE kind (`forbidden_keys`); `match_to_canonical` short-circuits to None when the source label is a canonical of the OPPOSITE axis. (2) `pipeline/stage4_merge.py` — `map_to_canonical_with_fallback` returns 'emerging' for opposite-axis canonicals. (3) `tests/test_taxonomy_disjointness.py` — added `test_synonym_index_kind_safe` + `test_match_to_canonical_no_cross_kind_coercion` (4/4 green). (4) `scripts/rederive_kindsafe_domains.py` — post-hoc re-derives `domains` from `domains_raw` (order-preserving); run on live checkpoint → **2,007 records (25.5%) fixed, 0 non-`domains` field drift**, output `checkpoint_enriched_kindsafe.jsonl` (original untouched, sha256 unchanged); **promoted to `checkpoint_enriched.jsonl` 2026-09-02** (backup `backup_pre_kindsafe_promote_20260902_005603`, sha256-verified). (5) **Config cleanup** — removed cross-kind aliases from `config/taxonomy_v5.yaml` (63 raw-alias lines) + `config/synonym_map.yaml` (18 synonym+keyword lines); `config/alias_map.yaml` verified ALREADY clean (0 cross-kind, no change). All configs backed up (`.bak_20260902_005929`). Code filter remains as defense-in-depth.
- **Files:** `pipeline/schemas.py`, `pipeline/stage4_merge.py`, `tests/test_taxonomy_disjointness.py`, `scripts/rederive_kindsafe_domains.py`
- **Source:** user "don't assume validate" challenge 2026-09-02 — verified against live taxonomy + `checkpoint_enriched.jsonl` (7,867 records).

## 🟢 BUG-199 — 2026-09-02 — D2399 promotion path has NO cross-kind disjointness guard: wrong-axis raw labels can be promoted into the OPPOSITE canonical set (re-blooms D2422/BUG-151 at runtime) — FIXED (cross-kind guard wired)
- **Symptom:** `taxonomy_manager.py` promotion path (`update_counts_from_fbs` → `check_for_replacements` → `apply_replacements`) files raw labels by FIELD POSITION, not by true kind. `discipline_raw='graphic design'` (a canonical DOMAIN emitted wrongly as discipline) is filed as a `label_type='discipline'` long-tail challenger; at count ≥ `EMERGING_FREQ_THRESHOLD` (10) it auto-promotes to `emerging`, then `check_for_replacements` (ratio 1.1) proposes promoting it to CANONICAL discipline — while `graphic design` is ALREADY a canonical domain ⇒ canonical cross-axis collision.
- **Root cause:** `_is_long_tail(raw_label, canon_set, syn_index)` only checks the SAME kind's canonical/synonym sets — it never asks "does this label belong to the OTHER kind?" The kind-constrained disjointness logic (D2133 `_accept` filter, `match_to_canonical(label, kind)`) IS applied at S4 remap time (canonical fields), but is NOT applied to raw labels entering the D2399 `taxonomy_counts` pipeline. `check_for_replacements`/`apply_replacements` have zero cross-kind guard. The only disjointness protection is the STATIC CI test (`tests/test_taxonomy_disjointness.py`, D2422) which checks `taxonomy_v5.yaml` at load time — it cannot see runtime promotions.
- **Impact:** ~184 distinct `discipline_raw` labels are actually DOMAINS (`graphic design` 193, `strategic management` 166, `business strategy` 142, `organizational behavior` 124, `human-computer interaction` 118, `software engineering` 110, …) and ~323 distinct `domains_raw` labels are actually DISCIPLINES (`product design` 540, `business strategy` 427, `public policy` 425, `software engineering` 379, …). All sit above `EMERGING_FREQ_THRESHOLD=10`. If `run_post_commit_taxonomy` (post-S6) runs as-is, it will propose promoting dozens of wrong-axis labels → canonical domain/discipline collision, silently re-introducing the exact BUG-167/D2422 reversal mistake at runtime.
- **Fix:** apply the kind-constrained logic to raw labels in the promotion path (the same logic already used for canonical fields): (1) in `update_counts_from_fbs`, before filing a raw label as a challenger, resolve its true kind via `match_to_canonical(label, kind)` and REJECT/skip it (or re-file under the correct axis + flag axis-swap) if it resolves to the OTHER kind; (2) strengthen `_is_long_tail` to also reject labels that map to the other kind's canonical/synonym set; (3) add a fail-loud cross-kind disjointness guard in `check_for_replacements`/`apply_replacements` (refuse to promote any label already canonical/emerging on the opposite axis). Must land BEFORE any D2399 promotion (pair with BUG-197 prompt fix + re-classification).
- **Fix applied (2026-09-02):** added `_is_opposite_kind(raw_label, kind)` helper + wired it into all three filing/promotion points — `update_counts_from_fbs` (discipline long-tail guard, domain long-tail guard) and `check_for_replacements` (skip cross-kind emerging candidates). Verified: `graphic design`/`organizational behavior`/`product design`/`software engineering` correctly flagged opposite-kind; genuine long-tail (`neuroaesthetics`, `image compression`) pass through.
- **Files:** `pipeline/taxonomy_manager.py` (`_is_long_tail`, `_is_opposite_kind`, `update_counts_from_fbs`, `check_for_replacements`, `apply_replacements`)
- **Source:** user challenge 2026-09-02 ("the problem with current S4 if raw domain and raw principle carries the same label and/or synonyms and gets promoted eventually they can conflict in canonical domain and discipline labels as well") — verified against live `checkpoint_enriched.jsonl` (7,867 records) + `taxonomy_manager.py` source.

## ✅ BUG-198 — 2026-09-01 — 6 S2 singleton principles emitted `application: 'None'` and were DROPPED at S4 (recoverable extraction gap) — RESOLVED (D2519: part 2 re-inject executed)
- **Symptom:** 6 principles extracted by S2 with full `definition`/`mechanism`/`boundary`/`consequence` but `application` = the literal string `'None'` — never reached the S4 checkpoint (D2371 `application` REQUIRED, fail-closed ⇒ quarantined/dropped at S4 routing). All 6 are substantive math/technique principles, not shells.
- **Records (fb_id):** Monte Carlo PDE Solution Method `f11b8327…`, Mathematical Notation Conventions `1b123adb…`, Maximum Entropy Principle in Decision Making `14978863…`, Hypnotic Trance Induction Method `2cae771b…`, Fibonacci Number Generation in Recipe Combinations `b4899671…`, Fibonacci Numbers and Determinant Properties `2b8ceef5…`.
- **Root cause:** S2 generator (Qwen3-Coder-30B) emitted the Python-repr `'None'` for the principle-only `application` field on 6 math/hypnosis passages (a generator gap, not a schema gap — the field is correctly REQUIRED by D2371).
- **Impact:** 6 valid extracted FBs silently lost. This is NOT "correctly quarantined" — D2371 worked as designed (blocked a None application), but the upstream S2 gap was never logged as recoverable, so the loss was silent (C16 class).
- **Recovery (post-hoc, NO rerun):** the 6 records are intact in `stage2_extract/t11/singleton_fbs.jsonl` + `checkpoint.jsonl` (fb_ids known). Backfill `application` via targeted re-extraction (`scripts/rerun_s2_targeted.py` on those 6 fb_ids) OR cross-family single-field fill (gemma), then re-inject pre-S5.
- **Future S2 fix:** schema-validity gate already rejects `None`-string for `application` — add an explicit "application empty → route to `_failed` recovery manifest" path so no extracted FB is silently dropped without a recoverable record.
- **Files:** `pipeline/stage2_extract.py` (generator prompt), `pipeline/stage4_merge.py` (D2371 routing), `knowledge pipeline/stage2_extract/t11/singleton_fbs.jsonl`
- **Source:** post-S4 forensic audit 2026-09-01 (this session) — corrects the earlier "6 empty-application = correctly quarantined, not a bug" framing.
- **RESOLVED (D2519, 2026-09-02):** part 2 EXECUTED — `scripts/reinject_bug198_singletons.py` ran the 6 recovered singletons through the real S4→S5→S6 path (`merged_cribs_classify`/`classify_depth_focused`/`deberta_check`/`insert_fb`), committing **1 PASS + 5 QUARANTINE** (single-source singletons fail NLI entailment as expected). DB **7,867 → 7,873**, `integrity_check` ok, foreign-key clean, R14 stamps correct. (D2520 then re-synced the S4 checkpoint to the DB to close the drift this re-inject caused.)

## 🔴 BUG-195 — 2026-09-01 — fb_id collision → S6 `INSERT OR REPLACE` silent data loss — POST-HOC FIXED (dedup_fb_id.py)
- **Symptom:** Live t11 checkpoint: 4 fb_id collision groups (9 records) — `Visual Technique Polarities (2)`, `Whale Hunting Process (2)`, `Compliment-driven Trait Amplification (2)`, `Situational Analysis…Grounded (2)/(3)`. S6 `INSERT OR REPLACE INTO fbs` (`fb_id TEXT PRIMARY KEY`) silently overwrites the earlier row → 5 FBs lost, `inserted` reports 9 while the DB holds 4.
- **Root cause (two modes, both confirmed):** (1) D2069 name-collision disambiguation renames with `" (N)"` suffix but preserves the S2 fb_id (D2350 "never re-hash") — the suffix does not change `sha256(name|definition)`. (2) S2→S4 name/definition normalization drift (D2350 preserves the S2 hash) collapses distinct FBs onto the same fb_id. In both cases the FBs are identical (name, definition) from DIFFERENT clusters — D2120 cosine dedup runs per-cluster, so cross-cluster near-duplicates survive.
- **Impact:** Silent data loss at commit (C16 violation — the exact class as BUG-188/D2487 truncation). gemma cross-family audit: all 9 are TRUE duplicates (should merge, not rename).
- **Fix (post-hoc, NOT an S4 rerun):** `scripts/dedup_fb_id.py` — groups by fb_id, verifies identical definition + name-differs-only-by-suffix, merges into one convergent FB (union source_clusters/source_ids/source_segments/source_books/source_authors/evidence_passages; is_convergent=True). Fail-closed (exit 1) on any genuine hash collision (different definitions). Stress-verified: 6,715→6,710, fb_id unique, idempotent, 6,706/6,706 non-collision records byte-identical.
- **Future S4 fix (for next batch):** cross-cluster dedup before name-collision suffixing, OR regenerate fb_id when a suffix is applied to a genuine distinct FB + add a duplicate-fb_id fail-loud guard in S6 before the upsert.
- **Files:** `scripts/dedup_fb_id.py`, `pipeline/stage4_merge.py` (D2069 ~L1961, D2350 ~L2030), `pipeline/stage6_commit.py` (L72, L372)
- **Source:** post-S4 forensic audit 2026-09-01 (this session)
- **D2504 CORRECTION:** the final post-finish `dedup_fb_id.py` run on the complete 7,874-record checkpoint merged **6 collision groups (13 records → 6, removed 7)** — the 4 above PLUS `Compounding Habit Gains (2)` and `Pineconing: Label and Address Fears (2)`, missed by the pre-finish partial-checkpoint audit. Net: 7,874 → 7,867.

## 🟢 BUG-203 — 2026-09-02 — S5 `FLAG` status DEAD CODE (never assigned; misleading "Disagree→FLAG (human): 0" report) — FIXED (D2504)
- **Symptom:** `stage5_verify.py` initialized `stats = {"PASS", "FLAG", "QUARANTINE"}` but S5 (DeBERTa-only, D2322) only ever emits PASS/QUARANTINE — `FLAG` was never assigned, yet the report printed `Disagree → FLAG (human): 0 FBs need review`, implying a human-review path that D2322 explicitly removed ("DeBERTa is the final authority — no human needed").
- **Root cause:** D2322 removed the human-adjudication FLAG path but left the stats init + icon + report line as dead remnants.
- **Fix (D2504):** deleted `FLAG` from `stats` init and the icon dict; replaced the misleading line with an honest `Quarantined needing human review: {sum(needs_human_review)}` count. Schema `VERIFICATION_STATUS = Literal["PASS", "FLAG", "QUARANTINE", "PENDING"]` (schemas.py) is LEFT INTACT — FLAG is a schema-legal status produced by other stages (e.g. stage2_relabel cross-family judge) and legacy data; downstream `status.py`/`query.py`/S6 correctly handle it.
- **Files:** `pipeline/stage5_verify.py`

## 🟢 BUG-204 — 2026-09-02 — `TAXONOMY_MAX_DISCIPLINES` default 72 vs live 75 + unused `_max_count` — FIXED (D2504)
- **Symptom:** `pipeline_paths.py:295` fallback default `max_disciplines` = 72, but `taxonomy_v5.yaml` has 75 canonical disciplines (config `max_disciplines: 75`). The `_max_count` loop var in `taxonomy_manager.check_for_replacements` was never read.
- **Root cause:** the D2378 "canonical cap" constants (`MAX_DOMAINS`/`MAX_DISCIPLINES`) were imported + aliased + looped but NEVER compared against anything — the closed-loop (promotion ⇒ demotion) preserves canonical cardinality by construction, so the cap was aspirational documentation, not enforced code (C19 dead code).
- **Fix (D2504):** `pipeline_paths.py` default 72→75; removed `MAX_DOMAINS`/`MAX_DISCIPLINES` aliases + their imports + `_max_count` loop var (loop now `for label_type in ("domain", "discipline")`).
- **Files:** `pipeline/pipeline_paths.py`, `pipeline/taxonomy_manager.py`

## ✅ BUG-205 — 2026-09-02 — `source_diversity` inflated by near-duplicate book files (S1/S2 book-dedup gap) — RESOLVED (D2507)
- **Symptom:** `source_diversity` over-counts distinct sources by treating near-duplicate book files as independent. Frontier-review validation (claude0041) found **20/50** sampled rows inflated: "SYSTEMANTICS. THE SYSTEMS BIBLE" vs "The Systems Bible" (same John Gall book, reissue title), "Thinking in Systems" (Meadows) listed under two filenames, "Reading Images" (Kress & van Leeuwen) under `liber3` + `z-lib` copies — in 2 of 5 CONTRA records the "2 sources" were literally the same file content under two names.
- **Root cause:** S1/S2 dedup operates on segment/sha256 + cluster level, NOT on book identity — different download sources/editions/reissue titles of the same book are not collapsed. `source_diversity` is `len(source_books)` after only partial dedup.
- **Impact:** `source_diversity >= 3` (the B2 synthesis discriminator) misclassifies effectively single/double-sourced records as multi-source synthesis, and inflates ISOR corroboration ratings. Blocks a trustworthy B2 synthesis-entailment fix.
- **Fix (D2507):** `pipeline/book_metadata.py` — `_unicode_fold()` (NFKC→NFKD→strip combining marks) in `normalize_author`/`normalize_title`/`_normalize_key`; `_SPACE_SUBTITLE_SPLIT` in `normalize_title` (space-separated subtitle opener collapses with the colon edition); `resolve_source_id` fallback key sanitized via `sanitize_source_book`. Measured **962 files → 910 distinct canonical source_ids**. 16/16 new tests.
- **Residual (NOT deterministic-hash-fixable — post-S6):** same-book title RENAME across editions ("Systemantics"=="The Systems Bible"), space-separated co-author lists ("Feigenbaum, Aria" vs "Feigenbaum Aria"), truncated metadata titles → require a Stage 0.5 metadata re-extraction + fuzzy title matcher.
- **Files:** `pipeline/book_metadata.py`, `tests/test_book_dedup_bug205.py`
- **Source:** frontier-review validation 2026-09-02 (claude0041/chatgpt0041), D2506 → D2507 fix

## 🟢 BUG-196 — 2026-09-01 — FB name truncation at `FB_NAME_MAX_WORDS=8` cuts mid-word — FIXED (D2517 code fix; names NOT backfilled, cosmetic)
- **Symptom:** ≥4 FB names cut mid-word: `"Situational Analysis in Practice Mapping Research with Grounded"` (missing "Theory"), `"The Critical First 30 Days: Focused Execution to"`, `"False Positive and False Negative Cost Trade-offs in"`, `"Successful People as Those Who Step Outside the"`.
- **Root cause:** `normalize_fb_name(max_words=FB_NAME_MAX_WORDS=8)` (stage4_merge.py:801-825) truncates `' '.join(words[:8])` with no ellipsis/sentinel. D2419 (2026-08-20) raised the cap 5→8 (BUG-149) but 8 words still truncates descriptive names. The truncation also collapses distinct concepts onto shared names, feeding BUG-195 collisions. **2nd sub-bug (found 2026-09-01):** the truncation branch slices the RAW `words` list, not the title-cased `normalized` list — `name = ' '.join(words[:max_words])` — so a truncated name REVERTS to the S2 raw casing (title-case just applied is silently undone).
- **Impact:** Data-quality (definition is intact; only the display name is truncated). Non-blocking for the current run (cosmetic); recoverable from the full name in S2.
- **Fix (future S4):** raise the cap (or make it config-first already is — `fb_name_max_words: 8`), add an ellipsis sentinel, and/or fix the upstream generator to emit concise names. Do NOT back-fill the current checkpoint (names are cosmetic).
- **Files:** `pipeline/stage4_merge.py` (normalize_fb_name), `config/pipeline_config.yaml` (`stage4.fb_name_max_words`)
- **Source:** post-S4 forensic audit 2026-09-01 (this session)

## 🟠 BUG-197 — 2026-09-01 — gpt-oss classifier emits DOMAIN labels as DISCIPLINE (domain-not-discipline) — KIND-SWAP EXECUTED (D2519) + prompt fix (D2510); residual re-classification deferred
- **Symptom:** ~15-20% of the 2,382 `emerging`-discipline records carry a canonical DOMAIN in `discipline_raw`: `graphic design` (134), `organizational behavior` (107), `data visualization` (52), `entrepreneurship` (30), `image compression`, `human resource management`. gemma cross-family double-audit confirms (classifies these raw labels as "domain", not "discipline").
- **Root cause:** the gpt-oss S4 classifier does not enforce D2422 domain/discipline disjointness — a domain label is emitted into `discipline`/`discipline_raw`, then fails to match any canonical discipline → `emerging`.
- **Impact:** inflates the discipline emerging rate (36.2% pre-remap); Phase 0 remap recovers only labels that map to a real canonical discipline, leaving the domain-not-discipline residue in `emerging`.
- **Fix (post-hoc, this run):** `remap_emerging.py` Phase 0a/0b resolves mappable labels; residual domain-not-discipline labels correctly stay `emerging` (they are NOT disciplines) and are reviewed at D2399 dynamic-promote time. Do NOT alias-promote a domain into a discipline (violates D2422 — the exact BUG-167/D2439 reversal mistake).
- **Fix (future S4):** strengthen the classifier prompt + `stage4_golden.yaml` to enforce the domain/discipline boundary, so domain labels land in `domains` and discipline labels in `discipline`.
- **Files:** `pipeline/stage4_merged_call.py` (classifier prompt), `config/golden/stage4_golden.yaml`, `scripts/remap_emerging.py`
- **QUANTIFIED 2026-09-01 (user data-side re-discovery):** the canonical `discipline`/`domains` fields are now **DISJOINT (0 hard cross-axis violations)** thanks to Phase 0 remap — but the **`*_raw` fields still carry the classifier's cross-axis leak**: **3,184 records (40.5%)** have a discipline-canonical label in `domains_raw` (`product design` 540, `software engineering` 379, `artificial intelligence` 87, `sociology` 83, …) and **422 records (5.4%)** have a domain-canonical label in `discipline_raw` (`graphic design` 193, `organizational behavior` 124, `data visualization` 56, `entrepreneurship` 35). Plus **2,080 records (26.4%)** remain `discipline: emerging`. So the classifier conflation is systemic (≈45.8% of records), NOT a few labels. The canonical axis is correct → **S5/S6 may proceed**; the raw leak is provenance noise that feeds D2399 promote/demote with garbage, so it MUST be fixed (prompt + re-classification) BEFORE any D2399 promotion.
- **EXECUTED (D2519, 2026-09-02):** deterministic kind-swap (`scripts/bug197_kind_swap.py`, LLM-free, backed up) — **3,694 FBs** field-scoped UPDATE (`domains`/`domains_raw`/`discipline`/`discipline_raw` ONLY). Direction A: **598 disciplines recovered** + **1,830 redundant leaks dropped**. Direction B: **374 domains recovered**. Verified **0 residual axis leaks** + integrity ok + `taxonomy_counts` reconciled. Prompt fix (D2510) verified present (`stage4_merged_call.py:46-53` DISCIPLINE-vs-DOMAIN disjointness block) → future S0-S6 batches protected at the classifier level. **Residual:** re-classification of the ~2,080 `emerging` + remaining raw-leak records = a future-batch LLM run (gpt-oss over thousands of records), NOT executed. D2399 promotions remain FROZEN (`d2399_promotions_frozen: true`, D2521).

## 🟠 BUG-194 — 2026-08-31 — `compute_fb_relationships` doesn't exclude `emerging` from `discipline_overlap` (BUG-188 twin) — FIXED
- **Symptom:** `discipline_sets` in `compute_fb_relationships` used the raw `discipline` value with no fallback exclusion, so FBs sharing the placeholder `discipline="emerging"` (~40% of T1.1 FBs) were linked via `discipline_overlap` — a false relationship (two FBs are not "related" merely because both sit outside the canonical taxonomy).
- **Root cause:** BUG-188 (D2487) added `related_fbs_exclude_domains` for `domain_overlap`, but the parallel `discipline_overlap` path never got the equivalent exclusion. `emerging` is a placeholder (`_canonical_*` maps no-match → "emerging"), not a real label.
- **Impact:** At ~40% emerging discipline, ~8M false `discipline_overlap` edges (priority 2) would crowd out real `domain_overlap` (priority 3) and `source_crossover` in the top-k selection, contaminating `related_fbs` (LightRAG graph foundation).
- **Fix:** `related_fbs_exclude_disciplines: [emerging]` (config, C12) → `S4_RELATED_FBS_EXCLUDE_DISCIPLINES` (pipeline_paths) → `exclude_disciplines` param + stripped from `discipline_sets`. Verified: two `emerging`-discipline FBs now yield empty discipline_sets (no false overlap).
- **Files:** `config/pipeline_config.yaml`, `pipeline/pipeline_paths.py`, `pipeline/stage4_merge.py`
- **Source:** pre-S4-commit forensic audit 2026-08-31 (relationship-graph placeholder-leak sweep)

## 🟢 BUG-192 — 2026-08-31 — `guard_stacks_single_source.py` substring match falsely flags legit `com.maxwell.omlx-api-key` — FIXED
- **Symptom:** `just preflight` failed at `monitor_stacks.py`: `[omlx] stale launchd agent LOADED: com.maxwell.omlx`. But `launchctl list` shows the loaded agent is `com.maxwell.omlx-api-key` (a legit one-shot `launchctl setenv OMLX_API_KEY sk-maxwell-local` agent, `LastExitStatus 0`), NOT the forbidden `com.maxwell.omlx` homebrew agent.
- **Root cause:** `guard_stacks_single_source.py` used `if label in listed` — a SUBSTRING test over the `launchctl list` output. `"com.maxwell.omlx" in "com.maxwell.omlx-api-key"` → True → false positive. The actual stale `com.maxwell.omlx` agent remains absent (D2455 held); only its prefix collided with the legit `-api-key` suffix.
- **Impact:** Preflight gate failed spuriously — a false alarm that would block the S4 commit and erode trust in the guard (a guard that cries wolf masks real regressions).
- **Fix:** exact whole-field match via `re.search(rf"(?m)(?:^|\s){re.escape(label)}\s*$", listed)` — matches the label as the last whitespace-delimited column of a `launchctl list` line. Verified: negative (no match on `-api-key` suffix), positive (matches `com.maxwell.omlx` and `homebrew.mxcl.omlx` exactly). Guard re-run: `✅ Single-source invariant holds for all stacks: ollama, omlx` (exit 0).
- **Files:** `scripts/guard_stacks_single_source.py`
- **Source:** preflight gate 2026-08-31 (pre-S4-commit forensic audit)

## 🟢 BUG-193 — 2026-08-31 — `CircuitOpenError` undefined in `discover_principles` (stage2_extract.py:1736) — FIXED (D2525)
- **Symptom:** ruff F821: `CircuitOpenError` undefined at `stage2_extract.py:1736` (`except CircuitOpenError:` inside `discover_principles`). The name is only imported via deferred `from pipeline.omlx_call import CircuitOpenError, ...` inside `call_llm` (line 1434) and `run_stage2` (2026) — NOT in `discover_principles`'s scope (1626-1757).
- **Root cause:** `discover_principles` uses `except CircuitOpenError: raise` (D2211 breaker-open abort) but never imports the name locally or at module level.
- **Impact:** If the OMLX circuit breaker opens during S2 discovery probing, the handler raises `NameError: name 'CircuitOpenError' is not defined` instead of cleanly re-raising — masking the real breaker-open condition (C16 fail-loud violation). **NON-BLOCKING for S4** (S2 already completed); cascades only on a future S2 re-run.
- **Fix (deferred):** add `from pipeline.omlx_call import CircuitOpenError` at the top of `discover_principles` (or a module-level import). Deferring to the next S2 run to avoid churn pre-S4-commit.
- **Files:** `pipeline/stage2_extract.py`
- **Source:** pre-S4-commit forensic audit 2026-08-31 (ruff F821 sweep)
...
- **Status:** ✅ FIXED (code, verified 2026-08-30) — S4 re-run required to recover ~5,322 lost FBs (7,874 principle FBs − 2,552 survivors). NOTE: the depth pre-pass `checkpoint.jsonl.depth.json` is DELETED at S4 completion (stage4_merge.py:2041), so the re-run is a FULL ~40-52h run — the depth phase is NOT skippable.
...
- **Follow-up (D2487 hardening, 2026-08-30):** 7 non-atomic `open('w')` writers converted to `safe_write`/`safe_write_jsonl` (bridge_s2_to_s4, stage1_5_fastembed, stage1_5_domain_cluster, probe_run, stage0_5_extract_metadata, golden_sampler, run_diagnostic); `stage0_convert.load_existing_checkpoint` made fail-closed (`load_jsonl`, D2332); `tools/benchmark_s4_depth.py` writes via `safe_write` (fsync+atomic). Added `scripts/preflight_checkpoint_check.py` (boundary + sha256 + record-count gate). 6-FB live smoke (`smoke_preflight_6fb`) passed 6/6 with preflight `--write-manifest` + `--check` EXIT=0.
- **Reader-side hardening (D2487, 2026-08-30):** resume now asserts `record_count` (persisted in `.state.json`) so a clean-cut `\n` truncation that `load_jsonl` cannot see is caught fail-closed; GE/PT/PI/TI sidecars stream via `safe_write_jsonl` (no single-join); `PrincipleJournal.append` uses `_write_all_bytes`+fsync. Remaining OPEN (logged in `aggregated_remaining_tasks.md`): wire `scripts/preflight_checkpoint_check.py` into S5 as a pre-consume gate; chunk/FAISS the O(n²) similarity matrices; fix the `dedup_fbs_by_cosine` short-embedding `IndexError`.
...
## 🟢 BUG-187 — 2026-08-29 — S4 FB record emits 7 fields not declared in `schemas.FB` + `jargon` key omission (schema drift, NON-BLOCKING) — FIXED (D2496; verified D2526)
...
## 🟢 BUG-180 — 2026-08-26 — mlx_provider.py system-prompt KV cache is dead stub code — DONE (file removed; verified D2526)
...
## 🟢 BUG-145 — 2026-08-18 — S2 model conflates extraction_type/content_type ('tool_instruction') — FIXED (D2417 repair + prompt disambiguation; verified D2526)
