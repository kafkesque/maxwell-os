...
## 🟡 BUG-173 — 2026-08-24 — `elaboration` non-empty on PT/GE contrary to ontology ("principle-only") — OPEN (drift, NOT a bug to fix now)
- **Symptom:** divergent-5type smoke: `process_template.elaboration` = 520 chars (render-workflow context), `growth_edge.elaboration` = 402 chars (hypothesis context). Ontology (`content_types.yaml` `core_body`) declares `elaboration` is "PRINCIPLE-ONLY (empty for PT/PI/GE/TI)". `process_instance` and `tool_instruction` are correctly empty — so the over-emission is PT/GE-only, matching S2's shared-skeleton prompt emitting elaboration opportunistically for narrative types.
- **Root cause:** S2 extraction prompt emits `elaboration` on the shared skeleton for all content types; the PT/GE single-source paths fill it with type-appropriate context (process edge-cases / hypothesis reasoning) rather than leaving it empty.
- **Impact:** NONE today — the elaboration is preserved raw on the sidecar JSONL but (a) never rendered as body (renderer `_TYPE_BODY` excludes elaboration for non-principles), (b) never committed (`commit_non_fb_types: false`). No contamination/leak/hidden-failure risk.
- **Verdict (D2446):** DOCUMENT, do NOT strip. Stripping `elaboration` in S4 would destructively drop S2-emitted context for zero gain. Reconcile at the next S2 prompt revision (either fold elaboration into a type-appropriate field for PT/GE or drop it from the non-principle skeleton).
- **Status:** 🟡 OPEN — documentation-level drift; revisit with S2 prompt revision (post-BUG-165).
...
- **Why NOT fix now (D2446 verdict):** (a) enriching uncommitted JSONL is pure bloat — no consumer; (b) `keywords` + classification require a NEW LLM classify path across 4 types (new failure surface for data nobody persists); (c) `fb_version`/runtime defaults are trivial but pointless until commit. → DEFER the whole enrichment atomically with the `commit_non_fb_types` decision (post-BUG-165).
- **F2 (2026-08-24):** the "stale stamps" half is now FIXED — sidecars re-stamped with S4 run traceability (`pipeline_run_id`/`pipeline_commit`/`routed_by_stage`) while preserving S2 `gen_model`. The "not classified/enriched" half stays deferred.
- **Status:** 🟡 OPEN — P2, deferred to post-BUG-165 (atomic with `commit_non_fb_types`).
- **Files:** pipeline/stage4_merge.py (routing path), config/pipeline_config.yaml (`commit_non_fb_types`), config/content_types.yaml (`metadata.classification`/`discovery`/`versioning`/`runtime`)
...
- **Root cause:** single-source S2 extraction did not emit `parameters` for this tool (possibly the tool has no parameters, or the extractor omitted them). Confirmed an S2 DATA gap, NOT an S4 code defect — S4 passes S2 fields through verbatim, so there is no safe S4-side fix.
- **Impact:** minor; single-source content, 1 of 143 TI records. Field audit (`scripts/audit_s4_fields.py`) flags it correctly.
- **Verdict (D2446):** no S4 fix exists. Verify on the full 143-record TI corpus during BUG-165 rerun; if systemic, the only safe fix surface is the S2 TI prompt (ensure `parameters` is in the required TI field list) — which is a re-extraction, deferred with BUG-165.
